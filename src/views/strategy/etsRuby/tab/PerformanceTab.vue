    ...
    <div>
        <v-row class="ma-0 info-card-container" :class="$wu.isMobile() ? 'mt-5' : 'mt-10'" justify="start" align="center">
            <v-col class="info-card-body-bottom">
                ...
                <ChartApy class="mx-n3" v-if="rateTab === 1" :data="apyDataEtsRuby" :usdPlusData="apyDataUsdPlusOp" ets-name="ETS RUBY"/>
                <ChartTvl class="mx-n3" v-if="rateTab === 3" :data="tvlDataEtsRuby" ets-name="ETS RUBY"/>
            </v-col>
        </v-row>
        ...
        <v-row align="center" justify="start" class="ma-0" :class="$wu.isMobile() ? 'mt-5' : 'mt-10'">
            ...
            <div v-if="!$wu.isMobile()" class="info-card-container-debank" @click="openLink('https://debank.com/profile/0x56Fad4F0095b35FD74eE8be48412dF49Bf90f29E')">
        ...
        <v-row v-if="$wu.isMobile()" align="center" justify="start" class="ma-0 mt-5">
            <div class="info-card-container-debank" style="width: 100% !important;" @click="openLink('https://debank.com/profile/0x56Fad4F0095b35FD74eE8be48412dF49Bf90f29E')">
        ...
        <v-row v-if="etsRubyStrategyData && etsRubyStrategyData.ownedAsset" align="start" justify="start" class="ma-0" :class="$wu.isMobile() ? 'mt-4' : 'mt-4'">
                ...
                <v-row class="info-card-container" justify="start" align="center">
                    <v-col class="info-card-body">
                        ...
                        <v-row class="info-row mt-8" justify="start" align="center">
                            ...
                            <v-spacer></v-spacer>
                            <label class="card-info-value">{{ etsRubyStrategyData ? etsRubyStrategyData.ownedAsset.token : '—' }}</label>
                        ...
                        <v-row class="info-row mt-8" justify="start" align="center">
                            ...
                            <label class="card-info-value">
                                {{ etsRubyStrategyData ? $utils.formatMoneyComma(etsRubyStrategyData.ownedAsset.amount, 0) : '—' }}
                                {{ etsRubyStrategyData ? etsRubyStrategyData.ownedAsset.token : '—' }}
                            </label>
                        </v-row>
                        <v-row class="info-row mt-8" justify="start" align="center">
                            ...
                            <v-spacer></v-spacer>
                            <label class="card-info-value">
                                {{ etsRubyStrategyData ? $utils.formatMoneyComma(etsRubyStrategyData.ownedAsset.amountUsdc, 0) : '—' }}
                                USD
                            </label>
                        </v-row>
                    </v-col>
                ...
                <v-row class="info-card-container" justify="start" align="center">
                    <v-col class="info-card-body">
                        ...
                        <v-row class="info-row mt-8" justify="start" align="center">
                            <label class="card-info mt-2">Token</label>
                            <v-spacer></v-spacer>
                            <label class="card-info-value">{{ etsRubyStrategyData ? etsRubyStrategyData.depositAsset.token : '—' }}</label>
                            <div class="info-card-icon ml-2">
                                <v-img :src="require('@/assets/currencies/usdc.png')"/>
                            </div>
                        </v-row>
                        <v-row class="info-row mt-8" justify="start" align="center">
                            ...
                            <v-spacer></v-spacer>
                            <label class="card-info-value">
                                {{ etsRubyStrategyData ? $utils.formatMoneyComma(etsRubyStrategyData.depositAsset.amount, 0) : '—' }}
                                {{ etsRubyStrategyData ? etsRubyStrategyData.depositAsset.token : '—' }}
                            </label>
                        </v-row>
                        <v-row class="info-row mt-8" justify="start" align="center">
                            <label class="card-info mt-2">Value</label>
                            <v-spacer></v-spacer>
                            <label class="card-info-value">
                                {{ etsRubyStrategyData ? $utils.formatMoneyComma(etsRubyStrategyData.depositAsset.amountUsdc, 0) : '—' }}
                                USD
                            </label>
                        </v-row>
                    </v-col>
                ...
                <v-row class="info-card-container" justify="start" align="center">
                    <v-col class="info-card-body">
                        ...
                        <v-row class="info-row mt-8" justify="start" align="center">
                            <label class="card-info mt-2">Token</label>
                            <v-spacer></v-spacer>
                            <label class="card-info-value">{{ etsRubyStrategyData ? etsRubyStrategyData.borrowedAsset.token : '—' }}</label>
                            <div class="info-card-icon ml-2">
                        ...
                        <v-row class="info-row mt-8" justify="start" align="center">
                            ...
                            <v-spacer></v-spacer>
                            <label class="card-info-value">
                                {{ etsRubyStrategyData ? $utils.formatMoneyComma(etsRubyStrategyData.borrowedAsset.amount, 0) : '—' }}
                                {{ etsRubyStrategyData ? etsRubyStrategyData.borrowedAsset.token : '—' }}
                            </label>
                        </v-row>
                        <v-row class="info-row mt-8" justify="start" align="center">
                            <label class="card-info mt-2">Value</label>
                            <v-spacer></v-spacer>
                            <label class="card-info-value">
                                {{ etsRubyStrategyData ? $utils.formatMoneyComma(etsRubyStrategyData.borrowedAsset.amountUsdc, 0) : '—' }}
                                USD
                            </label>
                        </v-row>
                    </v-col>
                ...
                <v-row class="info-card-container-bottom" justify="start" align="center">
                    <v-col class="info-card-body-bottom">
                        <v-row align="center">
                            ...
                            <label class="card-info-value ml-2">{{ (etsRubyStrategyData && etsRubyStrategyData.currentHealthFactor) ? ($utils.formatMoneyComma(etsRubyStrategyData.currentHealthFactor, 2)) : '—' }}</label>
                            ...
                            <label class="card-info-value ml-2">{{ (etsRubyStrategyData && etsRubyStrategyData.targetHealthFactor) ? ($utils.formatMoneyComma(etsRubyStrategyData.targetHealthFactor, 2)) : '—' }}</label>
                            <v-spacer></v-spacer>
                        </v-row>
                    </v-col>
                </v-row>
        ...
        <v-row class="ma-0 info-card-container" :class="$wu.isMobile() ? 'mt-5' : 'mt-6'" justify="start" align="center">
            <v-col class="info-card-body-bottom">
                <v-row align="center" justify="start" class="ma-0">
                    <label class="section-title-label">ETS RUBY payouts</label>
                </v-row>
                ...
                <v-row align="center" justify="center">
                    <v-col :cols="!$wu.isFull() ? 12 : 8">
                        <Table
                                v-if="!$wu.isMobile()"
                                profit-label="USDC per ETS RUBY"
                                :payout-data="etsRubyStrategyData ? [...etsRubyStrategyData.payoutItems].reverse() : []"/>
                    ...
                    <v-col :cols="!$wu.isFull() ? 12 : 4">
                        <Doughnut :size="280" color="#8a0d31" :last-date="lastPayoutDate"/>
                    </v-col>
                </v-row>
            </v-col>
        </v-row>
...
export default {
    ...
    computed: {
        ...mapGetters('marketData', ['etsRubyStrategyData', 'apyDataEtsRuby', 'tvlDataEtsRuby', 'apyDataUsdPlusOp']),
        ...
        lastPayoutDate: function () {
            return this.etsRubyStrategyData ? this.etsRubyStrategyData.payoutItems[this.etsRubyStrategyData.payoutItems.length - 1].payableDate : null;
        },
    },
