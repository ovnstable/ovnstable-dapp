...
const actions = {
  async upgradeToAction({ commit, dispatch, getters, rootState }, request) {
    ...
    await governor.methods
      .proposeExec([contract.options.address], [0], [abi], name)
      .send(params);
  },
  ...
  async updateGovernanceSettings(
    ...
    if (value.votingPeriod) {
      addresses.push(governor.options.address);
      abis.push(
        governor.methods.setVotingPeriod(value.votingPeriod).encodeABI()
      );
      values.push(0);
    }
    ...
    if (value.setProposalThreshold) {
      ...
      abis.push(
        governor.methods
          .setProposalThreshold(value.proposalThreshold)
          .encodeABI()
      );
      values.push(0);
    }
    ...
    if (value.updateQuorumNumerator) {
      ...
      abis.push(
        governor.methods
          .updateQuorumNumerator(value.updateQuorumNumerator)
          .encodeABI()
      );
      values.push(0);
    }
    ...
    await governor.methods
      .proposeExec(addresses, values, abis, name)
      .send(params);
  },
  ...
  async getSettings({ commit, dispatch, getters, rootState }) {
    ...
    let timeLockItem = {};
    timeLockItem.minDelay = await timelockController.methods
      .getMinDelay()
      .call();
    ...
    governorItem.proposalThreshold = await governor.methods
      .proposalThreshold()
      .call();
    governorItem.quorumNumerator = await governor.methods
      .quorumNumerator()
      .call();
  ...
  async changeFeeBuy({ commit, dispatch, getters, rootState }, request) {
    ...
    let abi = contract.methods
      .setBuyFee(request.fee, request.feeDenominator)
      .encodeABI();
    let name =
      "Proposal #" + getters.proposals.length + 1 + "Change set Buy Fee";
    await governor.methods
      .proposeExec([contract.options.address], [0], [abi], name)
      .send(params);
  },
  ...
  async minting({ commit, dispatch, getters, rootState }, request) {
    ...
    await governor.methods
      .proposeExec([govToken.options.address], [0], [abi], name)
      .send(params);
  },
  ...
  async getOverview({ commit, dispatch, getters, rootState }) {
    ...
    let totalDelegated = await govToken.methods
      .getVotes(rootState.accountData.account)
      .call();
  ...
  async checkAccount({ commit, dispatch, getters, rootState }, contractType) {
    ...
    if (!contractType || contractType === "USD+") {
      ...
      console.log(
        "Error when checkAccount. Contract type not found: ",
        contractType
      );
      return;
    }
  ...
  async setStrategiesM2MWeights(
    ...
    let pm;
    console.debug(
      "If contract type === USD+ in setStrategiesM2MWeights: ",
      contractType
    );
    if (!contractType || contractType === "USD+") {
      pm = rootState.web3.contracts.pm;
    ...
    for (let i = 0; i < weights.length; i++) {
      ...
      item.minWeight = new BigNumber(weight.minWeight)
        .multipliedBy(1000)
        .toNumber();
      item.targetWeight = new BigNumber(weight.targetWeight)
        .multipliedBy(1000)
        .toNumber();
      item.maxWeight = new BigNumber(weight.maxWeight)
        .multipliedBy(1000)
        .toNumber();
      item.enabled = weight.enabled;
      item.enabledReward = weight.enabledReward;
      item.riskFactor = new BigNumber(weight.riskFactor)
        .multipliedBy(1000)
        .toNumber();
    ...
    try {
      ...
      console.error(
        "Error with await pm.methods.setStrategyWeights(items).send(params):",
        e
      );
    }
  ...
  async estimateSetStrategiesM2MWeights(
    { commit, dispatch, getters, rootState },
    data
  ) {
    commit("setFinanceLoading", true);
    ...
    try {
      ...
      let estimateOptions = {
        from: account,
        gasPrice: rootState.gasPrice.gasPriceGwei
      };

      console.debug(
        "BlockNum, account and estimateOptions: ",
        blockNum,
        account,
        estimateOptions
      );
  ...
  async estimateRebalancePortfolio(
    { commit, dispatch, getters, rootState },
    contractType
  ) {
    commit("setFinanceLoading", true);
  ...
  async getStrategyWeights(
    ...
    for (let i = 0; i < strategiesMapping.length; i++) {
      ...
      let weight = weights.find(
        (value) =>
          strategy.address.toLowerCase() === value.strategy.toLowerCase()
      );
  ...
  async getM2M({ commit, dispatch, getters, rootState }, contractType) {
    ...
    if (contractType === "USD+") {
      ...
      console.log(
        "Error when get m2m. Contract type not found: ",
        contractType
      );
      return;
    }
    ...
    if (!tokenPlusContract || !pm) {
      console.log(
        "Error when get m2m. Contract not found: ",
        contractType,
        tokenPlusContract,
        pm
      );
    ...
    try {
      ...
      let url = apiUrl + `/${networkName}/${contractType.toLowerCase()}`;
      console.log(
        "Load strategies mapping when get m2m from url: ",
        url,
        strategyAssets
      );
      strategiesMapping = (await axios.get(url + "/dict/strategies")).data;
      console.log(
        "Loaded strategies mapping when get m2m: ",
        strategiesMapping
      );
    } catch (e) {
    ...
    for (let i = 0; i < strategyAssets.length; i++) {
      ...
      let mapping = strategiesMapping.find(
        (value) => value.address === asset.strategy
      );
      ...
      console.log(
        "rootState.network.assetDecimals: ",
        rootState.network.assetDecimals
      );

      let nav = fromAsset6
        ? numberUtils._fromE6(asset.netAssetValue.toString())
        : web3.web3.utils.fromWei(asset.netAssetValue, "ether");

      let targetWeights = weight.targetWeight
        ? parseInt(weight.targetWeight) / 1000
        : 0;
      ...
      items.push({
        ...
        netAssetValue: nav,
        liquidationValue: fromAsset6
          ? numberUtils._fromE6(asset.liquidationValue.toString())
          : web3.web3.utils.fromWei(asset.liquidationValue, "ether"),
    ...
    for (let i = 0; i < items.length; i++) {
      items[i].currentWeight = Number(
        ((100 * parseFloat(items[i].netAssetValue)) / sum).toFixed(3)
      );
    ...
    commit(
      "setM2mTotal",
      fromAsset6
        ? numberUtils._fromE6(totalNetAssets)
        : web3.web3.utils.fromWei(totalNetAssets, "ether")
    );

    if (tokenPlusContract) {
      console.log("tokenPlusContract--");
      let totalTokenPlus = numberUtils._fromE6(
        await tokenPlusContract.methods.totalSupply().call()
      );
      if (contractType === "ETH+") {
        totalTokenPlus = web3.web3.utils.fromWei(
          await tokenPlusContract.methods.totalSupply().call(),
          "ether"
        );
        console.log(typeof totalTokenPlus);
      }
      console.log(tokenPlusContract, "tokenPlusContract--2");
  ...
  async getRewardPools({ commit, dispatch, getters, rootState }) {
    ...
    pools.push({
      name: "Pool 1 mock",
      rewardRate: "10",
      periodFinish: "2022-05-11"
    });
    pools.push({
      name: "Pool 2 mock",
      rewardRate: "5",
      periodFinish: "2022-07-30"
    });
  ...
  async getAbroad({ commit, dispatch, getters, rootState }, contractType) {
    ...
    if (contractType === "USD+") {
      ...
      abroad.min = await rootState.web3.contracts.exchange.methods
        .abroadMin()
        .call();
      abroad.max = await rootState.web3.contracts.exchange.methods
        .abroadMax()
        .call();
    ...
    if (contractType === "DAI+") {
      ...
      abroad.min = await rootState.web3.contracts.daiExchange.methods
        .abroadMin()
        .call();
      abroad.max = await rootState.web3.contracts.daiExchange.methods
        .abroadMax()
        .call();
    ...
    if (contractType === "USDT+") {
      ...
      abroad.min = await rootState.web3.contracts.usdtExchange.methods
        .abroadMin()
        .call();
      abroad.max = await rootState.web3.contracts.usdtExchange.methods
        .abroadMax()
        .call();
    ...
    if (contractType === "ETH+") {
      ...
      abroad.min = await rootState.web3.contracts.wethExchange.methods
        .abroadMin()
        .call();
      abroad.max = await rootState.web3.contracts.wethExchange.methods
        .abroadMax()
        .call();
    ...
    console.log(
      "Error when get abroad. Contract type not found: ",
      contractType
    );
  },
  ...
  async updateAbroad({ commit, dispatch, getters, rootState }, abroad) {
    ...
    if (contractType === "USD+") {
      await rootState.web3.contracts.exchange.methods
        .setAbroad(abroad.min, abroad.max)
        .send(params);
      return;
    }

    if (contractType === "DAI+") {
      await rootState.web3.contracts.daiExchange.methods
        .setAbroad(abroad.min, abroad.max)
        .send(params);
      return;
    }
    ...
    if (contractType === "USDT+") {
      await rootState.web3.contracts.usdtExchange.methods
        .setAbroad(abroad.min, abroad.max)
        .send(params);
      return;
    }
    ...
    if (contractType === "ETH+") {
      await rootState.web3.contracts.wethExchange.methods
        .setAbroad(abroad.min, abroad.max)
        .send(params);
      return;
    }
    ...
    console.log(
      "Error when update abroad. Contract type not found: ",
      contractType
    );
  },
