<template>
  <v-col>
    ...
    <v-row class="mt-5">
      <v-spacer></v-spacer>
      <div class="swap-view-btn" @click="showUsdcRedeemView">
      ...
      <v-col cols="7">
        <v-row align="center" class="ma-0">
          <label class="balance-label ml-3"
            >Balance: {{ $utils.formatMoney(balance.usdcPlus, 3) }}</label
          >
          <div class="balance-network-icon ml-2">
            <v-img :src="icon" />
          </div>
    ...
    <v-row class="mt-5">
      <v-spacer></v-spacer>
      <label class="exchange-label">1 {{ assetName }} = 1 USDC+</label>
    </v-row>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("swapUsdcModal", ["assetUsdcApproved"]),
    ...
    maxResult: function() {
      return this.$utils.formatMoney(this.balance.usdcNative, 3);
    },

    sumResult: function() {
      this.sliderPercent =
        (parseFloat(this.sum) / parseFloat(this.balance.usdcNative)) * 100;
    ...
    buttonLabel: function() {
      ...
      } else if (this.sum > parseFloat(this.balance.usdcNative)) {
    ...
    numberRule: function() {
      ...
      if (
        !isNaN(parseFloat(v)) &&
        v >= 0 &&
        v <= parseFloat(this.balance.usdcNative).toFixed(6)
      )
        return true;
  ...
  methods: {
    ...mapActions("swapusdcData", ["refreshSwap"]),
    ...
    async changeSliderPercent() {
      this.sum =
        (this.balance.usdcNative * (this.sliderPercent / 100.0)).toFixed(
