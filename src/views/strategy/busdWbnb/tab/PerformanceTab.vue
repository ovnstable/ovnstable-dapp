    ...
    <div>
        <v-row class="ma-0 info-card-container" :class="$wu.isMobile() ? 'mt-5' : 'mt-10'" justify="start" align="center">
            <v-col class="info-card-body-bottom">
                ...
                <ChartApy class="mx-n3" v-if="rateTab === 1" :data="apyDataBusdWbnb" :usdPlusData="apyDataUsdPlusBinance" ets-name="BUSD/WBNB"/>
                <ChartTvl class="mx-n3" v-if="rateTab === 3" :data="tvlDataBusdWbnb" ets-name="BUSD/WBNB"/>
            </v-col>
        </v-row>
        ...
        <v-row align="center" justify="start" class="ma-0" :class="$wu.isMobile() ? 'mt-5' : 'mt-10'">
            ...
            <div v-if="!$wu.isMobile()" class="info-card-container-debank" @click="openLink('https://debank.com/profile/0x7802aa233497308989239BC3124d55Af57d9873C')">
        ...
        <v-row v-if="$wu.isMobile()" align="center" justify="start" class="ma-0 mt-5">
            <div class="info-card-container-debank" style="width: 100% !important;" @click="openLink('https://debank.com/profile/0x7802aa233497308989239BC3124d55Af57d9873C')">
        ...
        <v-row v-if="busdWbnbStrategyData.ownedAsset" align="start" justify="start" class="ma-0" :class="$wu.isMobile() ? 'mt-4' : 'mt-4'">
                ...
                <v-row class="info-card-container" justify="start" align="center">
                    <v-col class="info-card-body">
                        ...
                        <v-row class="info-row mt-8" justify="start" align="center">
                            ...
                            <v-spacer></v-spacer>
                            <label class="card-info-value">{{ busdWbnbStrategyData ? busdWbnbStrategyData.ownedAsset.token : '—' }}</label>
                        ...
                        <v-row class="info-row mt-8" justify="start" align="center">
                            ...
                            <label class="card-info-value">
                                {{ busdWbnbStrategyData ? $utils.formatMoneyComma(busdWbnbStrategyData.ownedAsset.amount, 0) : '—' }}
                                {{ busdWbnbStrategyData ? busdWbnbStrategyData.ownedAsset.token : '—' }}
                            </label>
                        </v-row>
                        <v-row class="info-row mt-8" justify="start" align="center">
                            ...
                            <v-spacer></v-spacer>
                            <label class="card-info-value">
                                {{ busdWbnbStrategyData ? $utils.formatMoneyComma(busdWbnbStrategyData.ownedAsset.amountUsdc, 0) : '—' }}
                                USD
                            </label>
                        </v-row>
                    </v-col>
                ...
                <v-row class="info-card-container" justify="start" align="center">
                    <v-col class="info-card-body">
                        ...
                        <v-row class="info-row mt-8" justify="start" align="center">
                            <label class="card-info mt-2">Token</label>
                            <v-spacer></v-spacer>
                            <label class="card-info-value">{{ busdWbnbStrategyData ? busdWbnbStrategyData.depositAsset.token : '—' }}</label>
                            <div class="info-card-icon ml-2">
                                <v-img :src="require('@/assets/currencies/busd.svg')"/>
                            </div>
                        </v-row>
                        <v-row class="info-row mt-8" justify="start" align="center">
                            ...
                            <v-spacer></v-spacer>
                            <label class="card-info-value">
                                {{ busdWbnbStrategyData ? $utils.formatMoneyComma(busdWbnbStrategyData.depositAsset.amount, 0) : '—' }}
                                {{ busdWbnbStrategyData ? busdWbnbStrategyData.depositAsset.token : '—' }}
                            </label>
                        </v-row>
                        <v-row class="info-row mt-8" justify="start" align="center">
                            <label class="card-info mt-2">Value</label>
                            <v-spacer></v-spacer>
                            <label class="card-info-value">
                                {{ busdWbnbStrategyData ? $utils.formatMoneyComma(busdWbnbStrategyData.depositAsset.amountUsdc, 0) : '—' }}
                                USD
                            </label>
                        </v-row>
                    </v-col>
                ...
                <v-row class="info-card-container" justify="start" align="center">
                    <v-col class="info-card-body">
                        ...
                        <v-row class="info-row mt-8" justify="start" align="center">
                            <label class="card-info mt-2">Token</label>
                            <v-spacer></v-spacer>
                            <label class="card-info-value">{{ busdWbnbStrategyData ? busdWbnbStrategyData.borrowedAsset.token : '—' }}</label>
                            <div class="info-card-icon ml-2">
                        ...
                        <v-row class="info-row mt-8" justify="start" align="center">
                            ...
                            <v-spacer></v-spacer>
                            <label class="card-info-value">
                                {{ busdWbnbStrategyData ? $utils.formatMoneyComma(busdWbnbStrategyData.borrowedAsset.amount, 0) : '—' }}
                                {{ busdWbnbStrategyData ? busdWbnbStrategyData.borrowedAsset.token : '—' }}
                            </label>
                        </v-row>
                        <v-row class="info-row mt-8" justify="start" align="center">
                            <label class="card-info mt-2">Value</label>
                            <v-spacer></v-spacer>
                            <label class="card-info-value">
                                {{ busdWbnbStrategyData ? $utils.formatMoneyComma(busdWbnbStrategyData.borrowedAsset.amountUsdc, 0) : '—' }}
                                USD
                            </label>
                        </v-row>
                    </v-col>
                ...
                <v-row class="info-card-container-bottom" justify="start" align="center">
                    <v-col class="info-card-body-bottom">
                        <v-row align="center">
                            ...
                            <label class="card-info-value ml-2">{{ (busdWbnbStrategyData && busdWbnbStrategyData.currentHealthFactor) ? ($utils.formatMoneyComma(busdWbnbStrategyData.currentHealthFactor, 2)) : '—' }}</label>
                            ...
                            <label class="card-info-value ml-2">{{ (busdWbnbStrategyData && busdWbnbStrategyData.targetHealthFactor) ? ($utils.formatMoneyComma(busdWbnbStrategyData.targetHealthFactor, 2)) : '—' }}</label>
                            <v-spacer></v-spacer>
                        </v-row>
                    </v-col>
                </v-row>
        ...
        <v-row class="ma-0 info-card-container" :class="$wu.isMobile() ? 'mt-5' : 'mt-6'" justify="start" align="center">
            <v-col class="info-card-body-bottom">
                <v-row align="center" justify="start" class="ma-0">
                    <label class="section-title-label">BUSD/WBNB payouts</label>
                </v-row>
                ...
                <v-row align="center" justify="center">
                    <v-col :cols="!$wu.isFull() ? 12 : 8">
                        <Table
                                v-if="!$wu.isMobile()"
                                profit-label="BUSD per BUSD/WBNB"
                                :payout-data="[...busdWbnbStrategyData.payoutItems].reverse()"/>
...
export default {
    ...
    computed: {
        ...mapGetters('marketData', ['busdWbnbStrategyData', 'apyDataBusdWbnb', 'tvlDataBusdWbnb', 'apyDataUsdPlusBinance']),
        ...
        lastPayoutDate: function () {
            return (this.busdWbnbStrategyData.payoutItems.length > 0) ? this.busdWbnbStrategyData.payoutItems[this.busdWbnbStrategyData.payoutItems.length - 1].payableDate : [];
        },
    },
