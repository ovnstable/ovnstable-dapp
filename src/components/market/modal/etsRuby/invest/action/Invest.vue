<template>
    <v-col>
        ...
        <v-row class="mt-8 mx-n3 main-card">
            <v-col>
                <v-row align="center" class="ma-0">
                    <label class="balance-label ml-3">Balance: {{ $utils.formatMoney(balance.etsRuby, 3) }}</label>
                    <div class="balance-network-icon ml-2">
                        <v-img :src="icon"/>
                    </div>
                </v-row>
        ...
        <v-row class="mt-5">
            <v-spacer></v-spacer>
            <label class="exchange-label">1 USD+ = 1 ETS RUBY</label>
        </v-row>
...
export default {
    ...
    computed: {
        ...
        buttonLabel: function () {
            ...
            } else if ((this.totalSupply.etsRuby) >= this.maxEtsRubySupply || (parseFloat(this.totalSupply.etsRuby) + parseFloat(this.sum)) >= parseFloat(this.maxEtsRubySupply)) {
        ...
        isBuy: function () {
            return this.account && this.sum > 0 && this.numberRule && (this.totalSupply.etsRuby < this.maxEtsRubySupply) && ((parseFloat(this.sum) + parseFloat(this.totalSupply.etsRuby)) < parseFloat(this.maxEtsRubySupply));
        },
    ...
    methods: {
        ...
        async buyAction() {
            ...
            this.showWaitingModal('Swapping ' + this.sumResult + ' USD+ for ' + this.sumResult + ' ETS RUBY');
            ...
            try {
                ...
                try {
                    ...
                    let buyResult = await contracts.etsExchangerRuby.methods.buy(sum, referral).send(buyParams);
                    ...
                    console.log(e);
                    this.closeWaitingModal();
                    this.showErrorModal('buyETSRuby');
                    return;
                }
        ...
        async checkAllowance(sum) {
            ...
            let allowanceValue = await contracts.usdPlus.methods.allowance(from, contracts.etsExchangerRuby.options.address).call();
            ...
            if (allowanceValue < sum) {
                try {
                    ...
                    let tx = await contracts.usdPlus.methods.approve(contracts.etsExchangerRuby.options.address, sum).send(approveParams);
        ...
        async estimateGas(sum) {
            ...
            try {
                ...
                let referral = await this.getReferralCode();
                debugger
                await contracts.etsExchangerRuby.methods.buy(sum, referral).estimateGas(estimateOptions)
                        ...
                        if (error && error.message) {
                            ...
                            let errorMsg = {
                                ...
                                data: {
                                    ...
                                    method: contracts.etsExchangerRuby.methods.buy(sum, referral).encodeABI(),
                                    message: msg,
                                    block: blockNum
                                }
                            };
