        ...
        <v-card-text>
          ...
          <v-row  justify="start">
            <label class="recent-label">
              Balance USDT: <b>{{ balances.usdt }}</b>
            </label>
          </v-row>

          <v-row justify="start">
            <label class="recent-label">
              Balance amUsdt: <b>{{ balances.amUsdt }}</b>
            </label>
          </v-row>

          <v-row justify="start">
            <label class="recent-label">
              Balance Static USDT: <b>{{ balances.staticAmUsdt }}</b>
            </label>
          </v-row>
          ...
          <v-row>
            <v-col>
              <v-row>
                <label class="title-row-label ml-5 mt-3">Amount USDT</label>
              </v-row>
              <v-row align="center">
            ...
            <v-col>
              <v-row>
                <label class="title-row-label ml-5 mt-3">Amount amUSDT</label>
              </v-row>
              <v-row align="center">
                <v-text-field placeholder="0.00"
                              @keypress="isNumber($event)"
      ...
      <v-card class="container_body">
        ...
        <v-card-text class="px-5">
          <v-col class="main-content-col">
            ...
            <v-row align="center" justify="center">
              <label class="recent-label">
                Balance StaticAmUSDT: <b>{{ balances.staticAmUsdt}}</b>
              </label>
            </v-row>
            ...
            <v-row class="row">
              <v-col cols="10">
                <div class="step-label">Swapping USDT to Static</div>
              </v-col>
              <v-col cols="2">
                <v-icon color="green" v-if="steps.swapUsdtToStatic">mdi-check</v-icon>
                <v-icon color="#8FA2B7" v-else>mdi-dots-horizontal</v-icon>
              </v-col>
            </v-row>
            ...
            <v-row class="row">
              <v-col cols="10">
                <div class="step-label">Approve USDT to LP</div>
              </v-col>
              <v-col cols="2">
                <v-icon color="green" v-if="steps.approveUsdtLp">mdi-check</v-icon>
                <v-icon color="#8FA2B7" v-else>mdi-dots-horizontal</v-icon>
              </v-col>
            </v-row>
            ...
            <v-row class="row">
              <v-col cols="10">
                <div class="step-label">Swap USDT to LP</div>
              </v-col>
              <v-col cols="2">
                <v-icon color="green" v-if="steps.swapUsdtToLp">mdi-check</v-icon>
                <v-icon color="#8FA2B7" v-else>mdi-dots-horizontal</v-icon>
              </v-col>
            </v-row>
...
export default {
  ...
  computed: {
    ...mapGetters('linearPoolUsdt', [ 'balances', "steps", ]
    ),
  ...
  methods: {
    ...mapActions('linearPoolUsdt', [ 'startProcess', 'updateBalances']),
    ...mapMutations('linearPoolUsdt', ['setAmountValueUsdt', 'setAmountValueAUsdt']),
    ...
    start() {
      ...
      this.setAmountValueUsdt(this.amountUsdt);
      this.setAmountValueAUsdt(this.amountAUsdt);
      this.startProcess();
    },

    close() {
      this.amountUsdt = null;
      this.amountAUsdt = null;
