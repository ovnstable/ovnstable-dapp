...
import UsdcPlusImage from "@/assets/usdcPlus.json";
...
const actions = {
  ...
  async addInsuranceToken({ commit, dispatch, getters, rootState }) {
    let option = {
      address:
        rootState.web3.contracts.insurance.optimism_token.options.address,
      symbol: "OVN INS",
      decimals: 18,
  ...
  async addUsdcPlusToken({ commit, dispatch, getters, rootState }) {
    const address = rootState.web3.contracts.usdcPlus.options.address;
    let option = {
      address: address,
      symbol: process.env.VUE_APP_USDC_TOKEN_NAME,
      decimals: 6,
      image: UsdcPlusImage.image
    };
