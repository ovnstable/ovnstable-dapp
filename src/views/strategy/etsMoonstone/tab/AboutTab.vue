    ...
    <div>
        <v-row align="center" justify="start" class="ma-0" :class="$wu.isMobile() ? 'mt-5' : 'mt-10'">
            <label class="strategy-info-label">
                The exchange-traded strategy Moonstone ($MOONSTONE), is an ERC-20 structured product built on Polygon that lets you leverage a collateralized debt position (USDC lent on AAVE) to borrow a volatile asset (WMATIC), pair it with USDC stablecoin, and provide WMATIC/USDC liquidity on Quickswap all in one action. This allows earning high APY and hedging against WMATIC volatility.
                <br/><br/>
                The unique feature of ETS MOONSTONE is that it automatically administers a health factor of {{ (etsMoonstoneStrategyData && etsMoonstoneStrategyData.targetHealthFactor) ? $utils.formatMoneyComma(etsMoonstoneStrategyData.targetHealthFactor, 2) : 1.2}}x on AAVE and rebalances your Lent/Borrowed amounts to maintain a stringent {{ (etsMoonstoneStrategyData && etsMoonstoneStrategyData.targetHealthFactor) ? $utils.formatMoneyComma(etsMoonstoneStrategyData.targetHealthFactor, 2) : 1.2}}x Health Factor. Payouts are happening every day and are auto compounded back into the strategy to further amplify rewards.
                <br/><br/>
                ...
                <v-row class="info-card-container" justify="start" align="center">
                    <v-col class="info-card-body">
                        ...
                        <v-row align="center">
                            ...
                            <label v-if="!$wu.isHuge()" class="info-card-text" :class="$wu.isMobile() ? 'ml-7' : 'ml-8'">Health Factor of {{ (etsMoonstoneStrategyData && etsMoonstoneStrategyData.targetHealthFactor) ? $utils.formatMoneyComma(etsMoonstoneStrategyData.targetHealthFactor, 2) : 1.2}}</label>

                            <label v-if="$wu.isHuge()" class="info-card-text mt-4 ml-2">Automatically monitored and managed Health Factor of {{ (etsMoonstoneStrategyData && etsMoonstoneStrategyData.targetHealthFactor) ? $utils.formatMoneyComma(etsMoonstoneStrategyData.targetHealthFactor, 2) : 1.2}}</label>
                        </v-row>
                        <v-row align="center">
                            <div class="info-card-icon mt-4">
                                <v-img :src="require('@/assets/icon/checkbox.svg')"/>
        ...
        <v-row align="start" justify="start" class="ma-0 mt-7">
            <v-col :cols="$wu.isMobile() ? 12 : 5">
                <v-row justify="start" align="start">
                    ...
                    <v-col class="ml-3">
                        ...
                        <v-row class="mt-8">
                            <label class="list-sub-title-text">Deposit USDC into the vault. This triggers an automatic borrow of value of WMATIC (from AAVE v3 markets), which is invested alongside your USDC deposit into Quickswap’s WMATIC/USDC pool.</label>
                        </v-row>
                    </v-col>
                </v-row>
                <v-row justify="start" align="start" class="mt-15">
                    ...
                    <v-col class="ml-3">
                        ...
                        <v-row class="mt-8">
                            <label class="list-sub-title-text">Strategy automatically stakes the earned pool LP tokens into Quickswap and auto-compounds the rewards by re-investing into the pool for more LP tokens to maximise Yields.</label>
                        </v-row>
                    </v-col>
                </v-row>
                <v-row justify="start" align="start" class="mt-15">
            ...
            <v-col cols="6" v-if="!$wu.isMobile()">
                <v-row justify="end">
                    <v-img class="scheme-img" max-width="700px" :src="require('@/assets/market/scheme_ets_moonstone.svg')"/>
                </v-row>
            </v-col>
        </v-row>

        <v-row align="center" justify="start" class="ma-0 mt-15" v-if="$wu.isMobile()">
            <v-img class="scheme-img" :src="require('@/assets/market/scheme_ets_moonstone.svg')"/>
        </v-row>
        ...
        <v-row class="ma-0 mt-7" justify="start" align="start">
            <v-col>
                <v-row class="info-row" justify="start" align="center">
                    ...
                    <v-spacer></v-spacer>
                    <label class="card-info-value">{{ (etsMoonstoneStrategyData && etsMoonstoneStrategyData.apy) ? ($utils.formatMoneyComma(etsMoonstoneStrategyData.apy, 1)) + '%' : '—' }}</label>
                ...
                <v-row class="info-row mt-6" justify="start" align="center">
                    ...
                    <v-spacer></v-spacer>
                    <label class="card-info-value">{{ (etsMoonstoneStrategyData && etsMoonstoneStrategyData.diffApy) ? ((etsMoonstoneStrategyData.diffApy >= 0 ? '+' : '') + $utils.formatMoneyComma(etsMoonstoneStrategyData.diffApy, 1)) + '%' : '—' }}</label>
                ...
                <v-row class="info-row mt-6" justify="start" align="center">
                    ...
                    <v-spacer></v-spacer>
                    <label class="card-info-value" :class="totalSupply.etsMoonstone >= maxEtsMoonstoneSupply ? 'label-error' : ''">
                ...
                <v-row class="info-row mt-6" justify="start" align="center">
                    ...
                    <v-spacer></v-spacer>
                    <label class="card-info-value">{{ (etsMoonstoneStrategyData && etsMoonstoneStrategyData.holders) ? $utils.formatMoneyComma(etsMoonstoneStrategyData.holders, 0) : '—' }}</label>
                </v-row>

                <v-row class="info-row" justify="start" align="center" v-if="$wu.isMobile()">
                    <label class="card-info mt-1">Target Health Factor</label>
                    <v-spacer></v-spacer>
                    <label class="card-info-value">{{ (etsMoonstoneStrategyData && etsMoonstoneStrategyData.targetHealthFactor) ? ($utils.formatMoneyComma(etsMoonstoneStrategyData.targetHealthFactor, 2)) : '—' }}</label>
                ...
                <v-row class="info-row mt-6" justify="start" align="center" v-if="$wu.isMobile()">
                    ...
                    <v-spacer></v-spacer>
                    <label class="card-info-value">{{ (etsMoonstoneStrategyData && etsMoonstoneStrategyData.healthFactorCheckInterval) ? etsMoonstoneStrategyData.healthFactorCheckInterval : '—' }}<label style="text-transform: none">&nbsp;hours</label></label>
                    <Tooltip text="What is Health Factor?" link="https://docs.aave.com/risk/asset-risk/risk-parameters#health-factor"/>
                </v-row>
            </v-col>
        ...
        <v-row align="center" justify="start" class="ma-0 mt-7 info-container">
            <v-col :class="!$wu.isFull() ? '' : 'ml-5'" :cols="!$wu.isFull() ? 12 : 0">
                <v-row justify="center" align="center" @click="openTokenOnScan(etsMoonstoneStrategyData.rebaseAddress)">
                    <label class="address-card-text" :class="!$wu.isFull() ? 'ml-2' : ''">Token address</label>
                    <v-spacer v-if="!$wu.isFull()"></v-spacer>
                    <label class="address-card-link ml-3">{{ (etsMoonstoneStrategyData && etsMoonstoneStrategyData.rebaseAddress) ? shortAddress(etsMoonstoneStrategyData.rebaseAddress) : '—' }}</label>
            ...
            <v-col :cols="!$wu.isFull() ? 12 : 0">
                <v-row justify="center" align="center" @click="openStrategyOnScan(etsMoonstoneStrategyData.exchangerAddress)">
                    <label class="address-card-text" :class="!$wu.isFull() ? 'ml-2' : ''">Pool address</label>
                    <v-spacer v-if="!$wu.isFull()"></v-spacer>
                    <label class="address-card-link ml-3">{{ (etsMoonstoneStrategyData && etsMoonstoneStrategyData.exchangerAddress) ? shortAddress(etsMoonstoneStrategyData.exchangerAddress) : '—' }}</label>
                    <div class="icon-img ml-2" :class="!$wu.isFull() ? 'mr-2' : ''">
                        <v-img :src="require('@/assets/icon/openBlue.svg')"/>
                    </div>
                </v-row>
            ...
            <v-col :cols="!$wu.isFull() ? 12 : 0">
                <v-row justify="center" align="center" @click="openStrategyOnScan(etsMoonstoneStrategyData.strategyAddress)">
                    <label class="address-card-text" :class="!$wu.isFull() ? 'ml-2' : ''">Vault address</label>
                    <v-spacer v-if="!$wu.isFull()"></v-spacer>
                    <label class="address-card-link ml-3">{{ (etsMoonstoneStrategyData && etsMoonstoneStrategyData.strategyAddress) ? shortAddress(etsMoonstoneStrategyData.strategyAddress) : '—' }}</label>
                    <div class="icon-img ml-2" :class="!$wu.isFull() ? 'mr-2' : ''">
                        <v-img :src="require('@/assets/icon/openBlue.svg')"/>
                    </div>
                </v-row>
        ...
        <template v-if="networkSupport">
            <v-row align="center" justify="start" class="ma-0" :class="$wu.isMobile() ? 'mt-10 mb-10' : 'mt-15'">
                <v-btn class="header-btn btn-filled" :class="(totalSupply.etsMoonstone >= maxEtsMoonstoneSupply) ? 'disabled-btn' : ''" @click="mintAction" :disabled="totalSupply.etsMoonstone > maxEtsMoonstoneSupply">
                    MINT ETS MOONSTONE
                </v-btn>
                <template v-if="totalSupply.etsMoonstone >= maxEtsMoonstoneSupply">
                    <label v-if="$wu.isFull()" class="full-status-error-label ml-4">TVL > ${{ $utils.formatMoneyComma(maxEtsMoonstoneSupply, 0) }}. Please check status later.</label>
                </template>
            </v-row>

            <v-row align="center" justify="center" class="ma-0" :class="$wu.isMobile() ? 'mt-n8' : 'mt-2'" v-if="!$wu.isFull()">
                <template v-if="totalSupply.etsMoonstone >= maxEtsMoonstoneSupply">
                    <label class="full-status-error-label">TVL > ${{ $utils.formatMoneyComma(maxEtsMoonstoneSupply, 0) }}. Please check status later.</label>
                </template>
            </v-row>
        </template>
...
export default {
    ...
    computed: {
        ...
        exitFee: function () {
            if (this.etsMoonstoneStrategyData && this.etsMoonstoneStrategyData.fees) {
                let result = this.etsMoonstoneStrategyData.fees.find(x => x.id === 'redeem');
                return result ? result.value : null;
            } else {
                return null;
            }
        ...
        performanceFee: function () {
            if (this.etsMoonstoneStrategyData && this.etsMoonstoneStrategyData.fees) {
                let result = this.etsMoonstoneStrategyData.fees.find(x => x.id === 'profit');
                return result ? result.value : null;
            } else {
                return null;
            }
        ...
        managementFee: function () {
            if (this.etsMoonstoneStrategyData && this.etsMoonstoneStrategyData.fees) {
                let result = this.etsMoonstoneStrategyData.fees.find(x => x.id === 'tvl');
                return result ? result.value : null;
            } else {
                return null;
            }
