<template>
    <v-col>
        ...
        <v-row class="mt-5">
            <v-spacer></v-spacer>
            <div class="swap-view-btn" @click="showEthRedeemView">
        ...
        <v-row class="mt-8 mx-n3 main-card">
            <v-col>
                <v-row align="center" class="ma-0">
                    <label class="balance-label ml-3">Balance: {{ $utils.formatMoney(balance.ethPlus, 3) }}</label>
                    <div class="balance-network-icon ml-2">
                        <v-img :src="icon"/>
                    </div>
                </v-row>
      ...
      <v-row class="mt-5">
            <v-spacer></v-spacer>
            <label class="exchange-label">1 {{ assetName }} = 1 ETH+</label>
        </v-row>
...
export default {
    ...
    computed: {
        ...
        maxResult: function () {
            return this.$utils.formatMoney(this.balance.weth, 3);
        },

        sumResult: function () {
            this.sliderPercent = parseFloat(this.sum) / parseFloat(this.balance.weth) * 100;
        ...
        buttonLabel: function () {
            ...
            } else if (this.sum > parseFloat(this.balance.weth)) {
        ...
        transactionPending: function () {
            return this.transactions.filter(value => (value.pending && (value.chain === this.networkId) && (value.product === 'ethPlus') && (value.action === 'mint'))).length > 0;
        },
        ...
        numberRule: function () {
            ...
            if (!isNaN(parseFloat(v)) && v >= 0 && v <= parseFloat(this.balance.weth).toFixed(6)) return true;
    ...
    methods: {
        ...
        ...mapActions("swapEthModal", ['showEthRedeemView', 'approveAsset', 'disapproveAsset']),
        ...
        async changeSliderPercent() {
            this.sum = (this.balance.weth * (this.sliderPercent / 100.0)).toFixed(this.sliderPercent === 0 ? 0 : 6) + '';
