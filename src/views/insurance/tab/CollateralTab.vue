  ...
  <div>
    ...
    >
      <v-col class="info-card-body-bottom">
        ...
        <v-row class="ma-0 mt-5 align-center"> </v-row>
    ...
    <v-row v-if="!isCurrentTotalDataLoading" class=" ma-0 mt-3">
        ...
        >
          <div>
            <v-img
              class="currency"
              :src="
                require(`@/assets/currencies/insurance/round_insurance_${chainName}.svg`)
              "
            />
          </div>
      ...
      <v-row class="ml-1 pt-6">
        <v-col class="col-12 col-lg-6 col-md-6 col-sm-12">
          ...
          <template v-else>
            <v-row align="start" justify="start" class="mt-0">
              <v-btn
                class="header-btn btn-investor-invest"
                @click="setWalletNetwork(chainName)"
              >
        ...
        <v-col class="col-12 col-lg-6 col-md-6 col-sm-12"> </v-col>
      </v-row>
    </v-row>
...
import {
  OVN_INS_CONTRACT_ADDRESS_OPTIMISM,
  OVN_INS_CONTRACT_ADDRESS_ARBITRUM
} from "@/utils/const.js";
...
export default {
  ...
  computed: {
    ...
    networkSupport: function() {
      return this.networkName === this.chainName ? this.networkName : null;
    },

    tabNetworkName: function() {
      let params;
      params = this.getParams(this.chainName);
    ...
    explorerLink: function() {
      ...
      if (this.tabNetworkName === "arbitrum") {
        return `https://arbiscan.io/token/${OVN_INS_CONTRACT_ADDRESS_ARBITRUM}`;
      }

      console.error(
        "Not found networkId type when return INSURANCE usd+ explorer link"
      );
      return null;
    },
    ...
    contractAddress: function() {
      ...
      if (this.tabNetworkName === "arbitrum") {
        return OVN_INS_CONTRACT_ADDRESS_ARBITRUM;
      }

      console.error(
        "Not found networkId type when return INSURANCE usd+ contract address"
      );
      return null;
    }
  },
