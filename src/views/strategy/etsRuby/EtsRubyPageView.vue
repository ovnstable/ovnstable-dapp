                ...
                <v-col :cols="$wu.isFull() ? 9 : 12" :class="$wu.isFull() ? 'ml-n3' : ''">
                    ...
                    <v-row class="info-card-container-white ma-0 mt-5" justify="start" align="center" v-if="!$wu.isFull()">
                        <v-col class="my-10 mx-8">
                                ...
                                <v-row align="center">
                                    <v-col cols="12">
                                        ...
                                        <v-row align="center" class="mt-5">
                                            <label class="investor-card-sub-title-value">{{ this.balance.etsRuby ? ($utils.formatMoneyComma(this.balance.etsRuby, 2) + ' USD+') : '—' }}</label>
                                        </v-row>
                                    </v-col>
                                </v-row>
                                <v-row align="center">
                                    <v-col cols="12">
                                        ...
                                        <v-row class="info-row mt-6" justify="start" align="center">
                                            ...
                                            <v-spacer></v-spacer>
                                            <label class="investor-card-sub-title-value" :class="clientProfitDayEtsRuby > 0 ? 'success-color' : ''">
                                                {{ clientProfitDayEtsRuby ? ((clientProfitDayEtsRuby > 0 ? '+' : '') + '$' + $utils.formatMoneyComma(clientProfitDayEtsRuby, 4)) : '—' }}
                                            </label>
                                        </v-row>
                                        <v-row class="info-row mt-6" justify="start" align="center">
                            ...
                            <template v-if="networkSupport">
                                <v-row align="center" justify="center" class="ma-0" :class="$wu.isMobile() ? 'mt-10' : 'mt-12'" v-if="totalSupply.etsRuby >= maxEtsRubySupply">
                                    <label class="full-status-error-label">TVL > ${{ $utils.formatMoneyComma(maxEtsRubySupply, 0) }}. Please check status later.</label>
                                </v-row>

                                <v-row align="center" justify="center" class="ma-0" :class="(totalSupply.etsRuby >= maxEtsRubySupply) ? 'mt-2' : 'mt-12'">
                                    <v-btn class="header-btn btn-investor-invest" :class="(totalSupply.etsRuby >= maxEtsRubySupply) ? 'disabled-btn' : ''" :disabled="totalSupply.etsRuby >= maxEtsRubySupply"  @click="mintAction">
                                        MINT ETS RUBY
                                    </v-btn>
                                </v-row>
                ...
                <v-col cols="3" v-if="$wu.isFull()">
                    ...
                    <v-row class="info-card-container-white ma-0 sticky mt-15" justify="start" align="center" style="width: 20%;">
                        <v-col class="my-10 mx-8">
                            ...
                            <template v-if="networkSupport">
                                <v-row align="center" justify="center" class="ma-0 mt-12" v-if="totalSupply.etsRuby >= maxEtsRubySupply">
                                    <label class="full-status-error-label">TVL > ${{ $utils.formatMoneyComma(maxEtsRubySupply, 0) }}. Please check status later.</label>
                                </v-row>
...
import StrategyBanner from "@/components/market/strategy/section/etsRuby/StrategyBanner";
import RiskDisclosureModal from "@/components/market/modal/etsRuby/RiskDisclosureModal";
import {mapActions, mapGetters} from "vuex";
import Tooltip from "@/components/common/element/Tooltip";
import AboutTab from "@/views/strategy/etsRuby/tab/AboutTab";
import PerformanceTab from "@/views/strategy/etsRuby/tab/PerformanceTab";
...
export default {
    ...
    computed: {
        ...
        ...mapGetters('marketData', ['etsRubyStrategyData', 'clientProfitDayEtsRuby']),
        ...mapGetters('marketUI', ['showEtsRuby']),
    ...
    methods: {
        ...
        ...mapActions('investModal', ['showEtsRubyModal', 'showMintView', 'showRedeemView']),
