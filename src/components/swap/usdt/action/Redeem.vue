<template>
    <v-col>
        ...
        <v-row class="mt-5">
            <v-spacer></v-spacer>
            <div class="swap-view-btn" @click="showUsdtMintView">
        ...
        <v-row class="mt-8 mx-n3 main-card">
            <v-col>
                <v-row align="center" class="ma-0">
                    <label class="balance-label ml-3">Balance: {{ $utils.formatMoney(balance.usdt, 3) }}</label>
                    <div class="balance-network-icon ml-2">
                        <v-img :src="icon"/>
                    </div>
                </v-row>
        ...
        <v-row class="mt-5">
            <v-spacer></v-spacer>
            <label class="exchange-label">1 USDT+ = 1 {{ assetName }}</label>
        </v-row>
...
export default {
    ...
    computed: {
        ...
        ...mapGetters('swapUsdtModal', ['usdtPlusApproved']),
        ...
        maxResult: function () {
            return this.$utils.formatMoney(this.balance.usdtPlus, 3);
        },

        sumResult: function () {
            this.sliderPercent = parseFloat(this.sum) / parseFloat(this.balance.usdtPlus) * 100;
        ...
        buttonLabel: function () {
            ...
            } else if (this.sum > parseFloat(this.balance.usdtPlus)) {
        ...
        transactionPending: function () {
            return this.transactions.filter(value => (value.pending && (value.chain === this.networkId) && (value.product === 'usdtPlus') && (value.action === 'redeem'))).length > 0;
        },
        ...
        numberRule: function () {
            ...
            if (!isNaN(parseFloat(v)) && v >= 0 && v <= parseFloat(this.balance.usdtPlus).toFixed(6)) return true;
    ...
    methods: {
        ...
        ...mapActions("swapUsdtData", ['refreshSwap']),
        ...mapActions("swapUsdtModal", ['showUsdtMintView', 'approveUsdtPlus', 'disapproveUsdtPlus']),
        ...
        async changeSliderPercent() {
            this.sum = (this.balance.usdtPlus * (this.sliderPercent / 100.0)).toFixed(this.sliderPercent === 0 ? 0 : 6) + '';
            ...
            await this.checkApprove(
                ...
                this.sum,
                this.assetDecimals,
                this.contracts.usdtExchange,
                'redeem',
                this.contracts.usdt,
                this.disapproveUsdtPlus,
                this.approveUsdtPlus,
                this.contracts.usdtPlus
            );
        },
