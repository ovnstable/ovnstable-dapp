<template>
  <v-col>
    ...
    <v-row class="mt-5">
      <v-spacer></v-spacer>
      <div class="swap-view-btn" @click="showUsdcMintView">
      ...
      <v-col>
        <v-row align="center" class="ma-0">
          <label class="balance-label ml-3"
            >Balance: {{ $utils.formatMoney(balance.usdcNative, 3) }}</label
          >
          <div class="balance-network-icon ml-2">
            <v-img :src="icon" />
          </div>
    ...
    <v-row class="mt-5">
      <v-spacer></v-spacer>
      <label class="exchange-label">1 USDC+ = 1 {{ assetName }}</label>
    </v-row>
      ...
      <v-col>
        <v-row>
          ...
          <label class="action-info-sub-label ml-2">{{
            "$" +
              (estimateResult
                ? $utils.formatMoneyComma(estimateResult, 2)
                : "0")
          }}</label>
        </v-row>
      </v-col>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("swapUsdcModal", ["usdcPlusApproved"]),
    ...
    maxResult: function() {
      return this.$utils.formatMoney(this.balance.usdcPlus, 3);
    },

    sumResult: function() {
      this.sliderPercent =
        (parseFloat(this.sum) / parseFloat(this.balance.usdcPlus)) * 100;
    ...
    estimateResult: function() {
      return (
        this.sum *
        (1 - (this.overnightFee ? this.overnightFee / 100.0 : 0.0004))
      );
    },
    ...
    buttonLabel: function() {
      ...
      } else if (this.sum > parseFloat(this.balance.usdcPlus)) {
    ...
    numberRule: function() {
      ...
      if (
        !isNaN(parseFloat(v)) &&
        v >= 0 &&
        v <= parseFloat(this.balance.usdcPlus).toFixed(6)
      )
        return true;
  ...
  methods: {
    ...
    async changeSliderPercent() {
      this.sum =
        (this.balance.usdcPlus * (this.sliderPercent / 100.0)).toFixed(
      ...
      await this.checkApprove(
        ...
        this.sum,
        this.assetDecimals,
        this.contracts.usdcExchange,
        "redeem",
        this.contracts.usdcNative,
        this.disapproveUsdcPlus,
        this.approveUsdcPlus,
        this.contracts.usdcPlus
      );
    },
