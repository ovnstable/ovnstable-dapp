...
<!--                      <v-row class="ma-0 mt-3  d-flex justify-space-between info-container">
&lt;!&ndash;                      <v-row class="ma-0 mt-3 insurance-card-info-row d-flex justify-space-between info-container">&ndash;&gt;
                          ...
                          <div class="tooltip-info">
                            <Tooltip :size="$wu.isFull() ? 18 : ($wu.isTablet() ? 16 : 14)"
                                     :maxWidth="300"
                                     text="Retained portion of the OVN yield"/>
                          </div>
                        </v-row>-->
...
<!--                        <v-row class="ma-0 mt-3 insurance-card-info-row d-flex justify-space-between info-container">
                          ...
                          <div class="tooltip-info">
                            <Tooltip :size="$wu.isFull() ? 18 : ($wu.isTablet() ? 16 : 14)"
                                     ...
                                     :link="'https://app.overnight.fi/insurance/' + getParams(networkId).networkName"/>
                          </div>
                        </v-row>-->
                    </v-col>
                </v-row>
...
<!--                    <label class="card-info-value mt-2">{{ $utils.formatMoneyComma(totalInsuranceSupply.optimism, 2) }} OVN</label>-->
                    <label class="card-info-value mt-2">- OVN</label>
                </v-row>

                <v-row class="ma-0 insurance-card-info-row d-flex justify-space-between">
                    <label class="card-info-label mt-2">Value of vault</label>
<!--                    <label class="card-info-value mt-2">${{ $utils.formatMoneyComma(totalInsuranceSupply.optimism, 2) }}</label>-->
                    <label class="card-info-value mt-2">- $</label>
                </v-row>
...
export default {
    ...
    computed: {
        ...
        coverageValue: function () {
            if (this.totalInsuranceSupply && this.totalUsdPlusValue) {
                return 100.00 * this.totalInsuranceSupply.optimism / this.totalUsdPlusValue;
            } else {
                return null;
            }
        },
    ...
    methods: {
        ...
        loadAvgApyInfo() {
            this.isLoaded = false;
            insuranceApiService.getAvgApyInfo(this.getParams(this.networkId).appApiUrl, 'month')
            .then(data => {
