...
export default {
  ...
  methods: {
    ...
    updateHeaders() {
      if (
        this.networkId === 137 ||
        this.networkId === 42161 ||
        this.networkId === 8453 ||
        this.networkId === 59144
      ) {
        if (!this.headersM2M.find((value) => value.text === "Risk Factor")) {
          this.headersM2M.push({ text: "Risk Factor", value: "riskFactor" });
        }
      } else {
        this.headersM2M = this.headersM2M.filter(
          (value) => value.text !== "Risk Factor"
        );
      }
    },
    loadData() {
    ...
    loadApiTotalUsdPlusValue() {
      let url =
        this.apiUrl + `/${this.networkName}/${this.contractType.toLowerCase()}`;
      ...
      usdPlusApiService
        ...
        .catch((e) => {
          console.error(
            "Error when load " + this.contractType + " total in governance",
            e
          );
        });
    },
    async loadBlockchainTotalUsdPlusValue() {
      setTimeout(async () => {
        ...
        if (!contractTokenPlus) {
          console.log(
            "loadBlockchainTotalUsdPlusValue error, contractTokenPlus not found"
          );
          return;
        }

        let assetDecimals =
          (await contractTokenPlus.methods.decimals().call()) * 1;
        let blockchainValue = await contractTokenPlus.methods
          .totalSupply()
          .call();
        console.log("blockhain total usd:", blockchainValue);
        let fromAsset6 = assetDecimals === 6;
        this.totalUsdPlusValue = fromAsset6
          ? numberUtils._fromE6(blockchainValue.toString())
          : this.web3.utils.fromWei(blockchainValue, "ether");
      }, 3000);
    },
    ...
    getTokenPlusContract() {
      ...
      console.log(
        "Error when load blockchain total values contract type not found: ",
        this.contractType
      );
      return null;
    }
