<template>
    <v-col>
        ...
        <v-row class="mt-5">
            <v-spacer></v-spacer>
            <label class="exchange-label">1 ETS RUBY = 1 USD+</label>
        </v-row>
...
export default {
    ...
    data: () => ({
        currency: {
            id: 'etsRuby',
            title: 'ETS RUBY',
            image: require('@/assets/currencies/market/ets_ruby.svg')
        },
        ...
        sliderPercent: 0,
        stepLabels: ['', 'Approve ETS RUBY', 'Confirmation'],
        step: 0
    }),
    ...
    computed: {
        ...
        ...mapGetters('investModal', ['etsRubyApproved']),
        ...
        maxResult: function () {
            return this.$utils.formatMoney(this.balance.etsRuby, 3);
        },

        sumResult: function () {
            this.sliderPercent = parseFloat(this.sum) / parseFloat(this.balance.etsRuby) * 100;
        ...
        buttonLabel: function () {
            ...
            if (!this.account) {
                ...
                if (this.etsRubyApproved) {
                    this.step = 2;
                    return 'Confirm transaction'
                } else {
                    this.step = 1;
                    return 'Approve ETS RUBY';
                }
            } else if (this.sum > parseFloat(this.balance.etsRuby)) {
                return 'Withdraw'
            } else {
                return 'Withdraw';
            }
        ...
        numberRule: function () {
            ...
            if (!isNaN(parseFloat(v)) && v >= 0 && v <= parseFloat(this.balance.etsRuby)) return true;
    ...
    methods: {
        ...
        ...mapActions("investModal", ['showMintView', 'approveEtsRuby']),
        ...
        changeSliderPercent() {
            this.sum = (this.balance.etsRuby * (this.sliderPercent / 100.0)).toFixed(this.sliderPercent === 0 ? 0 : 6) + '';
        },
        ...
        async redeemAction() {
            ...
            this.showWaitingModal('Withdrawing ' + this.sumResult + ' ETS RUBY for ' + this.sumResult + ' USD+');
            ...
            try {
                ...
                try {
                    ...
                    let buyResult = await contracts.etsExchangerRuby.methods.redeem(sum).send(buyParams);
                    ...
                    console.log(e)
                    this.closeWaitingModal();
                    this.showErrorModal('withdrawETSRuby');
                    return;
                }
        ...
        async checkAllowance(sum) {
            ...
            let allowanceValue = await contracts.etsRubyToken.methods.allowance(from, contracts.etsExchangerRuby.options.address).call();
            ...
            if (allowanceValue < sum) {
                try {
                    ...
                    let tx = await contracts.etsRubyToken.methods.approve(contracts.etsExchangerRuby.options.address, sum).send(approveParams);
        ...
        async estimateGas(sum) {
            ...
            try {
                ...
                await contracts.etsExchangerRuby.methods.redeem(sum).estimateGas(estimateOptions)
                    ...
                    .catch(function (error) {
                        if (error && error.message) {
                            ...
                            let errorMsg = {
                                ...
                                data: {
                                    from: from,
                                    to: contracts.etsExchangerRuby.options.address,
                                    gas: null,
                                    gasPrice: parseInt(estimateOptions.gasPrice, 16),
                                    method: contracts.etsExchangerRuby.methods.redeem(sum).encodeABI(),
                                    message: msg,
                                    block: blockNum
                                }
                            };
