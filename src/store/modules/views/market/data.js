...
const actions = {
      ...
      rootState.etsAction.etsList.map(async (ets) => {
        ...
        await fetch(
          ...
          .then((value) => {
            ...
            strategyData.timeData.sort((o1, o2) =>
            ...
            for (let i = 0; i < clientDataPreferment.length; i++) {
              ...
              try {
                ...
                if (clientDataLengthCounter === 1) {
                  ...
                  dispatch("addCompoundData", {
                    name: refreshParams.strategyName,
                    data: compoundData
                  });
                }
  ...
  async refreshUsdPlusPayoutsData({ commit, dispatch, getters, rootState }) {
    await Promise.all(
      ["polygon", "bsc", "optimism", "arbitrum", "zksync"].map(
        async (network) => {
          let appApiUrl;
