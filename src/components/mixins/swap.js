...
export const swap = {
  ...
  methods: {
    ...
    async checkApprove(
      ...
      console.log(
        `Check Approve action. Sum: ${sum} sliderPercent: ${sliderPercent} ActionDecimals: ${actionDecimals}`
      );
      console.log(
        `Check Approve action. Contracts: `,
        exchangeContract,
        actionContract
      );
      ...
      try {
        ...
        if (sliderPercent === 100) {
          ...
          switch (action === "swap-redeem" ? 6 : actionDecimals) {
            ...
            default:
              console.error(
                `Decimals not found when check approve. account: ${account} sum: ${sum} actionDecimals: ${actionDecimals}`
              );
              return;
          }
        }

        let allowApprove = await this.checkAllowance(
          action,
          account,
          contractSum,
          exchangeContract,
        ...
        console.log(
          `Allow to approve. allowApprove: ${allowApprove}, sum: ${sum}, contractSum: ${contractSum}, contractSumNum: ${contractSum *
            1}, originalBalance: ${originalBalance}`,
          allowApprove
        );
        if (!allowApprove) {
          disapproveActionFunc();
        ...
        console.error(
          `Approve action error. Type: ${action}. Account: ${account}, Sum: ${sum}. Error: ${e}`
        );
    ...
    async clearApprove(
      ...
      try {
        ...
        if (
          ...
          action === "market-redeem" ||
          action === "unwrap-redeem"
        ) {
          console.log(
            "Redeem clear allowance with ovnStableContract",
            ovnStableContract,
            allowanceValue
          );
          await ovnStableContract.methods
            .decreaseAllowance(exchangeContract.options.address, allowanceValue)
            .send(buyParams)
          ...
          console.log(
            "Action clear contract allowance",
            actionContract,
            allowanceValue
          );
          await actionContract.methods
            .decreaseAllowance(exchangeContract.options.address, allowanceValue)
            .send(buyParams)
            .on("transactionHash", (hash) => {
              console.log("Success clear allowance. hash: ", hash);
    ...
    async approveAction(
      ...
      try {
        ...
        switch (action === "swap-redeem" ? 6 : actionDecimals) {
          ...
          default:
            console.error(
              `Decimals not found when check approve action. Account: ${account} actionDecimals: ${actionDecimals}`
            );
            return;
        }
        ...
        allowApprove = !allowApprove
          ? await this.approveBlockchainAction(
              action,
              account,
              sum,
              exchangeContract,
        ...
        console.error(
          `Approve action error: ${e}. Type: ${action}. Account: ${account}. Error: ${e}`
        );
        disapproveActionFunc();
        this.showErrorModalWithMsg({ errorType: "approve", errorMsg: e });
      }
    ...
    async approveBlockchainAction(
      ...
      try {
        ...
        if (
          ...
          console.log(
            "Redeem approve with ovnStableContract",
            ovnStableContract
          );
          tx = await ovnStableContract.methods
            .approve(exchangeContract.options.address, sum)
            .send(approveParams);
        } else {
          console.log("Redeem approve with action contract", actionContract);
          tx = await actionContract.methods
            .approve(exchangeContract.options.address, sum)
            .send(approveParams);
        }
        ...
        console.error(
          `Approve blockchain action error. Type: ${action}. Sum: ${sum}. Account: ${account}. Error: ${e}`
        );
        this.showErrorModalWithMsg({ errorType: "approve", errorMsg: e });
        return false;
      }
    ...
    async getAllowanceValue(
      ...
      if (
        ...
        console.log(
          "Redeem allowance with ovnStableContract",
          ovnStableContract,
          exchangeContract
        );
        allowanceValue = await ovnStableContract.methods
          .allowance(from, exchangeContract.options.address)
          .call();
        console.log(
          "allowanceValue with ovnStable in redeem: ",
          allowanceValue
        );
      } else {
        console.log(
          "Action contract allowance",
          actionContract,
          exchangeContract
        );
        allowanceValue = await actionContract.methods
          .allowance(from, exchangeContract.options.address)
          .call();
        console.log("allowanceValue: ", allowanceValue);
      }
        ...
        await method.estimateGas(transactionData, (error, gasLimit) => {
          if (error) {
            ...
            this.web3.eth.getGasPrice().then((gasPrice) => {
              ...
              let feeInEther = this.web3.utils.fromWei(
                feeInWei.toString(),
                "ether"
              );
              console.log(
                "Swap  Estimated transaction fee in Ether:",
                feeInEther,
                feeInEther * 1854.91
              );
      ...
      try {
        ...
        if (sliderPercent === 100) {
          ...
          switch (actionDecimals) {
            ...
            default:
              console.error(
                `Decimals type not found for detect wei type in ${action}. Account: ${account} Sum: ${contractSum} SumInUsd: ${sumInUsd} ActionDecimals: ${actionDecimals}`
              );
              return;
          }
        }
        ...
        if (
          ...
          console.debug(
            `Buy-Action in ${action}. Approve not pass. Sum: ${contractSum} SumInUsd: ${sumInUsd}. Account: ${account}.`
          );
          return;
        }
        ...
        try {
          ...
          let method = await this.getContractMethodWithParams(
            action,
            account,
            contractSum,
            exchangeContract,
          ...
          if (!method) {
            let errorMessage =
              "Exchange Method type not found when create method params in buy action. MethodType: " +
              exchangeMethodName;
            console.error(errorMessage);
            this.showErrorModalWithMsg({
              errorType: "approve",
          ...
          console.debug(
            `Invest blockchain. Buy action Sum: ${contractSum}. decimals: ${actionDecimals}. usdSum: ${sum}. Account: ${account}. SlidersPercent: ${sliderPercent}`
          );
          let buyResult = await method
            .send(buyParams)
              ...
              if (self.networkName === "zksync" && self.zksyncFeeHistory) {
                setTimeout(async () => {
                  try {
                    ...
                    let weiBalance = await self.web3.eth.getBalance(
                      self.account
                    );
          ...
          console.error(
            `[Contract part method]. Action buy in ${action}. Sum: ${sum}. ContractSum: ${contractSum}, Account: ${account}. Error: ${e}`
          );
          this.showErrorModalWithMsg({ errorType: "approve", errorMsg: e });
        }
      } catch (e) {
        console.error(
          `[All method]. Action buy in ${action}. Sum: ${sum}. Account: ${account}. Error: ${e}`
        );
        this.showErrorModalWithMsg({ errorType: "approve", errorMsg: e });
      } finally {
        finalizeFunc();
    ...
    async confirmSwapAction(
      ...
      try {
        ...
        if (sliderPercent === 100) {
          ...
          if (!originalMax) {
            let errorMessage =
              "Original max value not exist, when swap action. type: " + action;
            console.error(errorMessage);
            this.showErrorModalWithMsg({
              errorType: "approve",
              errorMsg: { code: 1, message: errorMessage }
          ...
          switch (actionDecimals) {
            ...
            default:
              console.error(
                `Decimals type not found for detect wei type in ${action}. Account: ${account} Sum: ${contractSum} SumInUsd: ${sum} ActionDecimals: ${actionDecimals}`
              );
              return;
          }
        }
        ...
        let estimatedGasValue = await this.estimateGas(
          action,
          account,
          contractSum,
          "test-product",
    ...
    async estimateGas(
      ...
      try {
        ...
        if (!method) {
          let errorMessage =
            "Exchange Method type not found when create method params in estimate gas. MethodType: " +
            exchangeMethodName;
          console.error(errorMessage);
          this.showErrorModalWithMsg({
            errorType: "approve",
