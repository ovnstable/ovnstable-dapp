  ...
  <v-row class="card-container" @click="openStrategyCard">
    <v-col cols="12" align-self="start">
      <v-row
        ...
        justify="center"
        :style="{
          '--card-background': `linear-gradient(90deg, ${
      ...
      >
        <span class="currency ml-5">
          <v-img
            :src="
              require('@/assets/currencies/market/ets_' +
                cardData.data.name +
                '.svg')
            "
          />
          ...
          <v-row class="d-flex" align="center" style="width: 100% !important">
            <label class="card-title"
              ...
              {{
                `${
                  borrowProtocolCheck()
                    ? cardData.data.borrowProtocol + "&"
                    : ""
                }${cardData.data.dex} ${cardData.data.token2}/${
                  cardData.data.token1
                }`
              }}
            </label>
            <v-spacer></v-spacer>
      ...
      <v-container class="mt-3">
        <v-row align="center" justify="center" class="ma-0">
          <label
            class="full-status-error-label"
            v-if="
              cardData.overcapEnabled &&
              cardData.data.maxSupply &&
              cardData.tvl >= cardData.data.maxSupply
            "
            :style="{ color: cardData.data.mainColor }"
          >
        ...
        <template v-if="cardData.overcapEnabled">
          ...
          <v-row class="ma-0" align="center">
            <label
              class="progress-label-value mt-2"
              v-if="cardData.data"
              :style="
                cardData.data.maxSupply &&
                cardData.tvl >= cardData.data.maxSupply
    ...
    <v-col cols="12" align-self="end">
      <v-container>
            ...
            <v-row class="ma-0 mr-n2">
              <div class="box">
                ...
                <div class="icon">
                  <v-img
                    :src="
                      require('@/assets/network/' +
                        cardData.data.chainName +
                        '.svg')
                    "
                    class="mt-1"
                    alt="chain icon"
                ...
                <label class="chain-name mb-2 mt-1">{{
                  cardData.data.chainName
                }}</label>
              </div>
            </v-row>
          ...
          <v-col cols="6">
            <v-row class="ma-0 ml-n2">
              <div class="box">
                ...
                <label class="platform-name mb-2 mt-1">{{
                  cardData.data.dex
                }}</label>
              </div>
            </v-row>
            ...
            <v-row class="ma-0 mt-n4">
              <div class="box">
                ...
                <div class="icon">
                  <v-img
                    :src="
                      require('@/assets/cards/token_pair/' +
                        cardData.data.tokenPair +
                        '.svg')
                    "
                    class="mt-1"
                ...
                <label class="token-pair-name mb-2 mt-1">{{
                  cardData.data.poolName
                }}</label>
              </div>
            </v-row>
          </v-col>
        ...
        <v-row
          class="d-flex justify-space-between ma-0 mt-6"
          v-if="cardData.monthApy"
          align="center"
        >
            ...
            <label
              ...
              >{{
                dataHidden
                  ? ""
                  : accountEtsBalance
                  ? $utils.formatMoneyComma(accountEtsBalance, 2) +
                    " " +
                    cardData.data.actionTokenName
                  : "—"
              }}</label
            >
          </template>
          <template v-else>
            <v-spacer></v-spacer>
            <label class="your-deposit-disabled"
              >You don’t have deposit in this ETS</label
            >
            <v-spacer></v-spacer>
          </template>
        </v-row>
            ...
            <v-col class="ml-1">
              <v-row>
                <v-btn
                  ...
                  @click.stop="setWalletNetwork(cardData.chain.toString())"
                  outlined
                >
                  SWITCH TO {{ cardData.data?.chainName?.toUpperCase() }} TO
                  MINT
                </v-btn>
              </v-row>
            </v-col>
    ...
    <v-col cols="12" align-self="end">
      <v-row class="footer-row align-center justify-center">
        <label class="footer-link"
          ...
          {{
            `${
              borrowProtocolCheck() ? cardData.data.borrowProtocol + "&" : ""
            }${cardData.data.dex} ${cardData.data.token2}/${
              cardData.data.token1
            }`
          }}
...
export default {
  ...
  methods: {
    ...
    borrowProtocolCheck() {
      if (
        this.cardData.data.borrowProtocol !== undefined &&
        this.cardData.data.borrowProtocol !== null
      ) {
