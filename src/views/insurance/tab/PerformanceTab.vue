  ...
  <div>
    ...
    >
      <v-col class="info-card-body-bottom">
        <ChartApy
          class="mx-n3"
          :data="insurancePerformanceData.optimism"
...
export default {
  ...
  methods: {
        ...
        .then((value) => {
          ...
          for (let i = 0; i < clientDataPreferment.length; i++) {
            ...
            try {
              ...
              payout.comp = parseFloat(payout.comp ? payout.comp : 0.0).toFixed(
                3
              );

              widgetDataDict[this.$dayjs(payout.date).format("DD.MM.YYYY")] =
                payout.comp;
              // date
              if (i === 0) {
                compoundData.firstDate = this.$dayjs(payout.date).format(
                  "MMM D, YYYY"
                );
              }
              ...
              if (clientDataLengthCounter === 1) {
                ...
                compoundData.day = parseFloat(dayComp ? dayComp : 0.0).toFixed(
                  3
                );

                // week
                let weekComp = (accumulatorWeek * 100) / startValue - 100;
                compoundData.week = parseFloat(
                  weekComp ? weekComp : 0.0
                ).toFixed(3);

                // month
                let monthComp = (accumulatorMonth * 100) / startValue - 100;
                compoundData.month = parseFloat(
                  monthComp ? monthComp : 0.0
                ).toFixed(3);

                compoundData.all = payout.comp; // last payout comp

                this.addInsuranceCompoundData({
                  name: "optimism",
                  data: compoundData
                });
              }
              ...
              console.error(
                "strategyData build Widget Data Dict insurance error:",
                e
              );
            }
          }
          ...
          console.log(
            "InsuranceData: refreshStrategyData widgetData: ",
            widgetData
          );
          this.addInsurancePerformanceData({
            name: "optimism",
            data: widgetData
          });
    ...
    addInsuranceApyData(insuranceApyDataParams) {
      this.insuranceApyData[insuranceApyDataParams.name] =
        insuranceApyDataParams.data;
    },

    addInsurancePerformanceData(insurancePerformanceDataParams) {
      this.insurancePerformanceData[insurancePerformanceDataParams.name] =
        insurancePerformanceDataParams.data;
    },

    addInsuranceTvlData(insuranceTvlDataParams) {
      this.insuranceTvlData[insuranceTvlDataParams.name] =
        insuranceTvlDataParams.data;
    },

    addInsuranceCompoundData(compoundDataParams) {
      this.insuranceCompoundData[compoundDataParams.name] =
        compoundDataParams.data;
    }
  }
};
</script>
