<template>
  <div>
    <v-row v-if="mapCountOfFutures[networkName] > 1">
      ...
      >
        <v-btn
          :class="modalType === 'USD+' ? 'btn-outlined-enabled' : ''"
      ...
      >
        <v-btn
          :class="modalType === 'USDC+' ? 'btn-outlined-enabled' : ''"
          @click.stop="swapUsdcAction"
          class="button btn-outlined"
          outlined
        >
          USDC+
      ...
      <v-col
        v-if="wusdAvailibleNetworks.includes(networkName)"
        class="col-lg-3 col-md-3 col-sm-6"
      >
        <v-btn
          :class="modalType === 'WUSD+' ? 'btn-outlined-enabled' : ''"
      ...
      <v-col
        v-if="daiplusAvailibleNetworks.includes(networkName)"
        class="col-lg-3 col-md-3 col-sm-6"
      >
        <v-btn
          :class="modalType === 'DAI+' ? 'btn-outlined-enabled' : ''"
      ...
      <v-col
        v-if="usdtplusAvailibleNetworks.includes(networkName)"
        class="col-lg-3 col-md-3 col-sm-6"
      >
        <v-btn
          :class="modalType === 'USDT+' ? 'btn-outlined-enabled' : ''"
      ...
      <v-col
        v-if="ethplusAvailibleNetworks.includes(networkName)"
        class="col-lg-3 col-md-3 col-sm-6"
      >
        <v-btn
          :class="modalType === 'ETH+' ? 'btn-outlined-enabled' : ''"
      ...
      >
        <v-btn
          :class="modalType === 'WETH+' ? 'btn-outlined-enabled' : ''"
...
export default defineComponent({
  ...
  methods: {
    ...mapActions("swapModal", [
      "showSwapModal",
      "showMintView",
      "showRedeemView"
    ]),
    ...mapActions("swapDaiModal", [
      "showDaiSwapModal",
      "showDaiMintView",
      "showDaiRedeemView"
    ]),
    ...mapActions("swapUsdtModal", [
      "showUsdtSwapModal",
      "showUsdtMintView",
      "showUsdtRedeemView"
    ]),
    ...
    ...mapActions("swapEthModal", [
      "showEthSwapModal",
      "showEthMintView",
      "showEthRedeemView"
    ]),
    ...mapActions("wrapModal", [
      "showWrapModal",
      "showWrapView",
      "showUnwrapView"
    ]),
    ...mapActions("ethWrapModal", [
      "showEthWrapModal",
      "showEthWrapView",
      "showEthUnwrapView"
    ]),
    ...
    swapUsdcAction() {
      ...
      if (this.modalActionType === "MINT") {
        this.showUsdcSwapModal();
        this.showUsdcMintView();
        this.closeFunc();
        return;
      }

      this.showUsdcSwapModal();
      this.showUsdcRedeemView();
      this.closeFunc();
    },
    swapAction() {
