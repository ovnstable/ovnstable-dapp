<template>
  <v-menu transition="slide-y-reverse-transition" offset-y>
    <template v-slot:activator="{ on, attrs }">
      <div
        class="select-bar-main-container"
        ...
        <div class="btn-icon">
          ...
          <img
            :src="
              alphaNetwork
                ? light
                  ? require('@/assets/icon/alpha_black.svg')
                  : require('@/assets/icon/alpha_white.svg')
                : ''
            "
            alt="Alpha Icon"
            class="alpha"
          ...
          <img
            :src="
              betaNetwork
                ? light
                  ? require('@/assets/icon/beta_black.svg')
                  : require('@/assets/icon/beta_white.svg')
                : ''
            "
            alt="Beta Icon"
            class="beta"
          ...
          <img
            :src="
              light
                ? openedList
                  ? require('@/assets/icon/up_black.svg')
                  : require('@/assets/icon/down_black.svg')
                : openedList
                ? require('@/assets/icon/up_w.svg')
    ...
    <v-list class="network-select-list">
      ...
      <v-list-item style="cursor: pointer" @click="setWalletNetwork('59144')">
        ...
        <v-list-item-title class="network-select-list-item">
          <div style="display: inline-block">Linea</div>
          <div style="position: relative">
            <div class="tooltip-info">
              <Tooltip text="In the process of launch" top />
            </div>
          </div>
        </v-list-item-title>
      </v-list-item>
      ...
      <v-list-item>
        <div @click="openDeprecatedDocsLink()" class="title-deprecated pl-4">
          DEPRECATED
          <img
            :src="require('@/assets/icon/deprecated-link.svg')"
            width="13px"
...
.list-item-icon,
.btn-icon,
.selected-network-icon .v-image__image {
  width: 28px;
  height: 28px;
  margin-right: 10px;
}
