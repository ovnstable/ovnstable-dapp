  ...
  <v-col>
          ...
          <div class="coin-card mr-3">
            <v-row class="ma-2" align="center">
              <div class="coin-img mr-2">
                <v-img
                  :src="
                    require(`@/assets/currencies/insurance/round_insurance_${networkName}.svg`)
                  "
                />
              </div>
      ...
      <v-col>
        <v-row align="center" class="ma-0">
          <label class="balance-label ml-3"
            >Balance: {{ $utils.formatMoney(balance.ovn, 3) }}</label
          >
          <div class="balance-network-icon ml-2">
            <v-img :src="icon" />
          </div>
    ...
    <v-row class="mt-15" align="center" justify="center">
      ...
      >
        <v-col>
          <label
            @click="clearApprove('insurance-redeem')"
      ...
      <div class="action-btn-container" v-else>
        <template v-if="insuranceRedemptionData.request === 'NEED_WAIT'">
          ...
          >
            WITHDRAW IN
            {{ $utils.formatMoneyComma(insuranceRedemptionData.hours, 0) }}
            HOURS
          </v-btn>
        </template>
  ...
  computed: {
    ...
    maxResult: function() {
      return this.$utils.formatMoney(
        this.insuranceBalance[this.networkName],
        3
      );
    },
    ...
    sumResult: function() {
      this.sliderPercent =
        (parseFloat(this.sum) /
          parseFloat(this.insuranceBalance[this.networkName])) *
        100;

      if (!this.sum || this.sum === 0) return "0";
      else {
    ...
    buttonLabel: function() {
      ...
      if (!this.account) {
        ...
        this.sum > parseFloat(this.insuranceBalance[this.networkName])
      ) {
        return "Redeem";
      } else {
        return "Redeem";
    ...
    numberRule: function() {
      ...
      if (
        !isNaN(parseFloat(v)) &&
        v >= 0 &&
        v <= parseFloat(this.insuranceBalance[this.networkName]).toFixed(6)
      )
        return true;
  ...
  methods: {
    ...
    async changeSliderPercent() {
      this.sum =
        (
          this.insuranceBalance[this.networkName] *
          (this.sliderPercent / 100.0)
        ).toFixed(this.sliderPercent === 0 ? 0 : 6) + "";
    ...
    async checkApprove() {
      try {
        ...
        console.error(
          `Market Withdraw approve action error: ${e}. Sum: ${this.sum}. Account: ${this.account}. `
        );
    ...
    getMax() {
      let balanceElement = this.insuranceOriginalBalance[this.networkName];
    ...
    async clearApprove(action) {
      try {
        ...
        await contracts.insurance[`${this.networkName}_token`].methods
          .decreaseAllowance(
    ...
    async redeemAction() {
      try {
        console.debug(
          `Swap Withdraw blockchain. Start buy action. Account: ${this.account}. estimatedGasValue: ${this.sliderPercent}`
        );
        ...
        if (this.sliderPercent === 100) {
          ...
          if (!originalMax) {
            console.error(
              "Original max value not exist, when confirm redeem action in insurance redeem."
            );
            return;
          }
        } else {
        ...
        if (!(await this.checkApprove())) {
          console.debug(
            `Redeem insurance. Buy action Approve not pass. Sum: ${sum} usdSum: ${this.sum}. Account: ${this.account}.`
          );
          return;
        }
          ...
          await contracts.insurance[`${this.networkName}_exchanger`].methods
            ...
            .on("transactionHash", function(hash) {
              ...
              self.showSuccessModal({
                successTxHash: hash,
                successAction: "withdrawInsurance"
              });
              self.loadTransaction();
            });
        } catch (e) {
          console.error(
            `Withdraw Insurance blockchain redeem action error: ${e}. Sum: ${this.sum}. Account: ${this.account}. `
          );
          return;
        }
        ...
        console.error(
          `Withdraw Insurance redeem action error: ${e}. Sum: ${this.sum}. Account: ${this.account}. `
        );
      }
    },
    ...
    async confirmSwapAction() {
      try {
        ...
        if (this.sliderPercent === 100) {
          ...
          if (!originalMax) {
            console.error(
              "Original max value not exist, when confirm swap action in insurance redeem."
            );
            return;
          }
        } else {
        ...
        console.error(
          `Withdraw Insurance confirm swap error: ${e}. Sum: ${this.sum}. Account: ${this.account}. `
        );
    ...
    async approveAction() {
      try {
        ...
        console.error(
          `Withdraw Insurance approve error: ${e}. Sum: ${this.sum}. Account: ${this.account}. `
        );
        this.showErrorModal("approve");
      }
    },
    async approveBlockchainAction(sum) {
      try {
        ...
        let tx = await contracts.insurance[`${this.networkName}_token`].methods
          .approve(
            contracts.insurance[`${this.networkName}_exchanger`].options
              .address,
            sum
        ...
        console.error(
          `Withdraw Insurance allowance error: ${e}. Sum: ${this.sum}. Account: ${this.account}. `
        );
        return false;
      }
    },
    ...
    async getAllowanceValue() {
      ...
      let allowanceValue = await contracts.insurance[
        `${this.networkName}_token`
      ].methods
        .allowance(
          from,
    ...
    async estimateGas(sum) {
      ...
      try {
        ...
        await contracts.insurance[`${this.networkName}_exchanger`].methods
          ...
          .catch(function(error) {
            if (error && error.message) {
              ...
              console.error(
                `Withdraw Insurance blockchain redeem error: ${error}. Sum: ${this.sum}. Account: ${this.account}. `
              );
            }
        ...
        console.error(
          `Withdraw Insurance redeem error: ${e}. Sum: ${this.sum}. Account: ${this.account}. `
        );
        return -1;
      }
