...
export const zap = {
  data() {
    return {
      ...
      zapPlatformContractTypeMap: {
        // PlatformName: { name, type } name: ContractName, type: LP_WITH_STAKE_IN_ONE_STEP, LP_STAKE_DIFF_STEPS.
        Chronos: {
          name: "Chronos",
      ...
      poolsInfoMap: {
        // pool address : { gauge, poolId } poolId - id from dex, approveType - TOKEN, NFT
  ...
  methods: {
    ...
    async depositAllAtGauge(account, lastPoolInfoData, lastNftTokenId) {
      ...
      if (
        ...
        let balance = await this.poolTokenContract.methods
          .balanceOf(account)
          .call();
