...
export const odosSwap = {
  mixins: [tokenLogo, logger, errorHandler],
  data() {
    return {
      baseViewType: "SWAP",
  ...
  computed: {
    ...
    isAllLoaded: function() {
      ...
      if (this.baseViewType === "SWIPE") {
        return this.account
          ? this.isTokensLoadedAndFiltered && this.isFirstBalanceLoaded
          : this.isTokensLoadedAndFiltered;
      }
    ...
    stablecoinWithoutSecondTokens() {
      return this.stablecoinTokens.filter(
        (item) =>
          !this.stablecoinSecondTokens
  ...
  methods: {
    ...
    async initPoolSwap(listOfBuyTokensAddresses) {
      ...
      this.tokens = await this.getFilteredPoolTokens(
        networkId,
        false,
        [],
        true
      ); // [] - none execute
      this.secondTokens = await this.getFilteredPoolTokens(
        networkId,
        true,
    ...
    async initOvernightSwap() {
      ...
      this.secondTokens = await this.getFilteredOvernightTokens(
        networkId,
        true
      );
      this.isTokensLoadedAndFiltered = true;
    ...
    loadContractsForTokens(contractFile) {
      for (let i = 0; i < this.secondTokens.length; i++) {
        ...
        this.tokensContractMap[secondtoken.address] = this._loadContract(
          contractFile,
          this.web3,
          secondtoken.address
        );
      ...
      for (let i = 0; i < this.tokens.length; i++) {
        ...
        this.tokensContractMap[token.address] = this._loadContract(
          contractFile,
          this.web3,
          token.address
        );
        ...
        for (let i = 0; i < tokens.length; i++) {
          ...
          if (addresses.includes(token.address)) {
            ...
            await this.loadBalance(
              this.tokensContractMap[token.address],
              token
            );
          }
        }
    ...
    async loadBalance(contract, token) {
      ...
      try {
        ...
        if (token.address === "0x0000000000000000000000000000000000000000") {
          ...
          let balance = this.web3.utils.fromWei(
            ethBalance,
            this.getWeiMarker(token.decimals)
          );
          token.balanceData = {
            name: token.symbol,
            balance: balance,
    ...
    async loadContract(chainId) {
      ...
      return odosApiService
        ...
        .then((data) => {
          ...
          this.routerContract = this._loadContract(
            this.contractData.routerAbi,
            this.web3,
            this.contractData.routerAddress
          );
          this.executorContract = this._loadContract(
            this.contractData.erc20Abi,
            this.web3,
            this.contractData.executorAddress
          );
      ...
      this.loadPrices(chainId)
        .then(() => {
          ...
          for (let i = 0; i < tokens.length; i++) {
            ...
            try {
              token.estimatePerOne = this.web3.utils.fromWei(
                1 + "",
                this.getWeiMarker(token.decimals)
              );
            } catch (e) {
    ...
    async getFilteredPoolTokens(
      chainId,
      isIncludeInListAddresses,
      listTokensAddresses,
      ignoreBaseNetworkCurrency
    ) {
      let tokens = [];
        ...
        if (listTokensAddresses.length === tokens.length) {
          tokens = tokens.sort((a, b) => {
            const indexA = listTokensAddresses.findIndex(
              (item) => item.toLowerCase() === a.address.toLowerCase()
            );
            const indexB = listTokensAddresses.findIndex(
              (item) => item.toLowerCase() === b.address.toLowerCase()
            );
    ...
    async initWalletTransaction(
      ...
      let txLogMessage = this.getOdosLogMsg({
        message: "Odos send transaction",
        swapSession: this.swapSessionId,
        data: transactionData
      });
      txLogMessage +=
        " | Current block: " + (await this.web3.eth.getBlockNumber());
      console.debug(txLogMessage);
      const result = this.web3.eth
        ...
        .then(async (data) => {
          ...
          let addressesToUpdate = [...inputTokens, ...outputTokens].map(
    ...
    showSuccessModal(isShow, inputTokens, outputTokens, hash, txData) {
      this.successData = {
    ...
    updateTokenValue(token, value) {
      ...
      if (selectedToken) {
        let sum =
          token.decimals === 6 ? token.value * 100 + "" : token.value + "";
        token.contractValue = this.web3.utils.toWei(
          sum,
