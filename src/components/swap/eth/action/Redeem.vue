<template>
    <v-col>
        ...
        <v-row class="mt-5">
            <v-spacer></v-spacer>
            <div class="swap-view-btn" @click="showEthMintView">
        ...
        <v-row class="mt-8 mx-n3 main-card">
            <v-col>
                <v-row align="center" class="ma-0">
                    <label class="balance-label ml-3">Balance: {{ $utils.formatMoney(balance.weth, 3) }}</label>
                    <div class="balance-network-icon ml-2">
                        <v-img :src="icon"/>
                    </div>
                </v-row>
        ...
        <v-row class="mt-5">
            <v-spacer></v-spacer>
            <label class="exchange-label">1 ETH+ = 1 {{ assetName }}</label>
        </v-row>
...
export default {
    ...
    computed: {
        ...
        ...mapGetters('swapEthModal', ['ethPlusApproved']),
        ...
        sumResult: function () {
            this.sliderPercent = parseFloat(this.sum) / parseFloat(this.balance.ethPlus) * 100;
        ...
        buttonLabel: function () {
            ...
            } else if (this.sum > parseFloat(this.balance.ethPlus)) {
        ...
        transactionPending: function () {
            return this.transactions.filter(value => (value.pending && (value.chain === this.networkId) && (value.product === 'ethPlus') && (value.action === 'redeem'))).length > 0;
        },
        ...
        numberRule: function () {
            ...
            if (!isNaN(parseFloat(v)) && v >= 0 && v <= parseFloat(this.balance.ethPlus).toFixed(6)) return true;
    ...
    methods: {
        ...
        ...mapActions("swapEthData", ['refreshSwap']),
        ...mapActions("swapEthModal", ['showEthMintView', 'approveEthPlus', 'disapproveEthPlus']),
        ...
        async changeSliderPercent() {
            this.sum = (this.balance.ethPlus * (this.sliderPercent / 100.0)).toFixed(this.sliderPercent === 0 ? 0 : 6) + '';
            ...
            await this.checkApprove(
                ...
                this.sum,
                this.assetDecimals,
                this.contracts.wethExchange,
                'redeem',
                this.contracts.weth,
                this.disapproveEthPlus,
                this.approveEthPlus,
                this.contracts.ethPlus
            );
        },
