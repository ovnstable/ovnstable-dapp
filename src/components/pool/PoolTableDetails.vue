  ...
  <div>
    <div v-if="$wu.isMobile()" class="d-block zap-btn">
      <v-btn
        ...
        @click.stop="openZapInFunc(pool, 'pools')"
        outlined
      >
        ZAP IN
      </v-btn>
    ...
    <div class="row">
        ...
        <div class="pool-table-header-item">
          <div v-if="pool.isOpened" class="pool-table-platform-container">
            <div class="pool-platform-icon">
              <div class="icon mr-2">
                <v-img
                  :src="
                    require('@/assets/cards/platform/' + pool.platform + '.svg')
                  "
                  :title="pool.platform"
            ...
            <div
              v-if="pool.platform === 'Baseswapdefiedge'"
              class="pool-platform-name"
            >
              <div>
                Baseswap
              </div>
          ...
          <div v-else>
            <div class="pool-table-platform-container">
              <div @click="openPoolLink(pool, true)" class="pool-platform-icon">
                <div class="icon mr-2">
                  <v-img
                    :src="
                      require('@/assets/cards/platform/' +
                        pool.platform +
                        '.svg')
                    "
                    :title="pool.platform"
              ...
              <div
                v-for="(aggregator, index) in pool.aggregators"
                :key="aggregator.id"
      ...
      >
        <div class="pool-table-header-item text-center">
          <label
            v-if="
              pool.apr && !disabledAprInfoPlatformList.includes(pool.platform)
            "
            class="card-label"
          >
            {{ $utils.formatMoneyComma(pool.apr, 2) }}%
            <sup v-if="pool.platform === 'Beefy'">(apy)</sup>
          </label>
      ...
      >
        <div
          v-if="
            pool.tvl >= 1000000 &&
              !disabledTvlInfoPlatformList.includes(pool.platform)
          "
        ...
        <div
          v-if="disabledTvlInfoPlatformList.includes(pool.platform)"
          class="pool-table-header-item"
        >
      ...
      >
        <div @click="openLinkToStableFish" class="tooltip-wrapper">
          ...
          <div class="tooltip-content">
            <Tooltip top text="See on Stable Fish" :link="pool.stableFishUrl" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div
      v-if="
        pool.isOpened &&
          ...
          !isShowAprLimit &&
          pool.aggregators &&
          pool.aggregators.length
      "
    >
      <div class="aggregator-hr mt-4 mb-4"></div>

      <div
        v-for="aggregator in pool.aggregators"
        :key="aggregator.id"
        class="row"
      >
        ...
        <div class="col-3 col-xl-3 col-lg-3 col-md-3 col-sm-3">
          <div class="pool-table-header-item text-center">
            <label v-if="aggregator.apr" class="card-label text-center">
              {{ $utils.formatMoneyComma(aggregator.apr, 2) }}%<sup
                v-if="aggregator.platform === 'Beefy'"
                >apy</sup
              >
            </label>
            <label
