<template>
    <v-col>
        ...
        <v-row class="mt-5">
            <v-spacer></v-spacer>
            <label class="exchange-label">1 ETS MOONSTONE = 1 USD+</label>
        </v-row>
        ...
        <v-row class="mt-15" align="center" justify="center">
            ...
            <div class="action-btn-container" v-else>
                <v-btn v-if="etsMoonstoneApproved"
                       height="56"
                       class="buy"
...
export default {
    ...
    data: () => ({
        ...
        sliderPercent: 0,
        stepLabels: ['', 'Approve ETS MOONSTONE', 'Confirmation'],
        step: 0
    }),
    ...
    computed: {
        ...
        ...mapGetters('investModal', ['etsMoonstoneApproved']),
        ...
        maxResult: function () {
            return this.$utils.formatMoney(this.balance.etsMoonstone, 3);
        },

        sumResult: function () {
            this.sliderPercent = parseFloat(this.sum) / parseFloat(this.balance.etsMoonstone) * 100;
        ...
        buttonLabel: function () {
            ...
            if (!this.account) {
                ...
                if (this.etsMoonstoneApproved) {
                    this.step = 2;
                    return 'Confirm transaction'
                } else {
                    this.step = 1;
                    return 'Approve ETS MOONSTONE';
                }
            } else if (this.sum > parseFloat(this.balance.etsMoonstone)) {
                return 'Withdraw'
            } else {
                return 'Withdraw';
            }
        ...
        numberRule: function () {
            ...
            if (!isNaN(parseFloat(v)) && v >= 0 && v <= parseFloat(this.balance.etsMoonstone)) return true;
    ...
    methods: {
        ...
        ...mapActions("investModal", ['showMintView', 'approveEtsMoonstone']),
        ...
        changeSliderPercent() {
            this.sum = (this.balance.etsMoonstone * (this.sliderPercent / 100.0)).toFixed(this.sliderPercent === 0 ? 0 : 6) + '';
        },
        ...
        async redeemAction() {
            ...
            this.showWaitingModal('Withdrawing ' + this.sumResult + ' ETS MOONSTONE for ' + this.sumResult + ' USD+');
            ...
            try {
                ...
                try {
                    ...
                    let buyResult = await contracts.etsExchangerMoonstone.methods.redeem(sum).send(buyParams);
                    ...
                    console.log(e)
                    this.closeWaitingModal();
                    this.showErrorModal('withdrawETSMoonstone');
                    return;
                }
        ...
        async checkAllowance(sum) {
            ...
            let allowanceValue = await contracts.etsMoonstoneToken.methods.allowance(from, contracts.etsExchangerMoonstone.options.address).call();
            ...
            if (allowanceValue < sum) {
                try {
                    ...
                    let tx = await contracts.etsMoonstoneToken.methods.approve(contracts.etsExchangerMoonstone.options.address, sum).send(approveParams);
        ...
        async estimateGas(sum) {
            ...
            try {
                ...
                await contracts.etsExchangerMoonstone.methods.redeem(sum).estimateGas(estimateOptions)
                        ...
                        if (error && error.message) {
                            ...
                            let errorMsg = {
                                ...
                                data: {
                                    ...
                                    method: contracts.etsExchangerMoonstone.methods.redeem(sum).encodeABI(),
                                    message: msg,
                                    block: blockNum
                                }
                            };
