  ...
  <div>
    ...
    <div v-else class="mt-7 cards-list-container">
      ...
      <template v-if="sortedCardList.filter(value => (value.isPrototype || value.isOpenPrototype)).length > 0" >
        ...
        <EtsListCard class="mt-2"
                     v-for="component in sortedCardList.filter(value => (value.type === 'ets' && (value.isPrototype || value.isOpenPrototype)))"
                     :key="component.id"
                     :card-data="component"/>
      </template>
      ...
      <template v-if="sortedCardList.filter(value => value.isArchive).length > 0" >
        ...
        <template v-if="openedList">
          ...
          <EtsListCard class="mt-2"
                       v-for="component in sortedCardList.filter(value => (value.type === 'ets' && value.isArchive))"
                       :key="component.id"
                       :card-data="component"/>
        </template>
...
export default {
  ...
  methods: {
    ...
    async refreshMarket() {
      ...
      if (this.etsList && !this.isStartLoading) {
        try {
          ...
          this.refreshUsdPlusPayoutsData();
          this.refreshSupply();
    ...
    async refreshStrategyData() {
      console.log('MarketData: refreshStrategyData 1', this.etsList);
      ...
      await Promise.all(
          this.etsList.map(async ets => {
            ...
            console.log('MarketData: refreshStrategyData 2', appApiUrl);
            ...
            console.log('MarketData: refreshStrategyData: 5', refreshParams.strategyName);
          })
      );

      console.log('MarketData: refreshStrategyData: 6');
