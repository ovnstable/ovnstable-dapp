        ...
        <v-row justify="start">
          <div>
            <label class="mobile-info-title ml-5">
              {{
                compoundData && compoundData?.all
                  ? $utils.formatMoneyComma(compoundData?.all, 2) + "%"
                  : ""
              }}
            </label>
          </div>
          <div class="chart-sub-title-apy ml-5 mb-5">
            {{
              compoundData && compoundData.firstDate
                ? compoundData.firstDate
                : "-"
            }}
          </div>
        </v-row>
        ...
        <v-row>
          <v-col cols="3">
            <v-row justify="center">
              ...
              >
                {{
                  compoundData && compoundData?.day
                    ? $utils.formatMoneyComma(compoundData.day, 2) + "%"
                    : ""
                }}
              </label>
            </v-row>
            <v-row justify="center">
              <label class="chart-sub-title-apy"> 1 day </label>
            </v-row>
          </v-col>
          ...
          <v-col cols="3">
            <v-row justify="center" class="chart-title-compound-container">
              ...
              >
                {{
                  compoundData && compoundData?.week
                    ? $utils.formatMoneyComma(compoundData?.week, 2) + "%"
                    : ""
                }}
              </label>
            </v-row>
            <v-row justify="center">
              <label class="chart-sub-title-apy"> 1 week </label>
            </v-row>
          </v-col>
          ...
          <v-col cols="3">
            <v-row justify="center" class="chart-title-compound-container">
              <label
                :class="
                  compoundData?.month >= 0
                    ? 'chart-title-compound'
                    : 'chart-title-compound-minus'
                "
              >
                {{
                  compoundData && compoundData?.month
                    ? $utils.formatMoneyComma(compoundData?.month, 2) + "%"
                    : ""
                }}
              </label>
            </v-row>
            <v-row justify="center">
              <label class="chart-sub-title-apy"> 1 month </label>
            </v-row>
          </v-col>
          ...
          <v-col cols="3">
            ...
            <v-row justify="center" class="all-compound-container">
              <label class="chart-sub-title-apy"> All </label>

              <div class="tooltip-compound">
                <v-row align="center" justify="end">
                  <Tooltip
                    :size="16"
                    :icon-color="light ? '#ADB3BD' : '#707A8B'"
                    text="Cumulative return for the period"
                  />
                </v-row>
              </div>
            </v-row>
...
export default {
  ...
  methods: {
    ...
    async zoomChart(zoom) {
      ...
      await fetch(
        ...
        .then((value) => {
          ...
          if (this.avgApy.date) {
            this.avgApy.date = this.$dayjs(this.avgApy.date).format(
              "DD MMM. ‘YY"
            );
          }
        })
    ...
    changeZoomBtnStyle() {
      ...
      document
        .getElementById(this.zoom + "-zoom-btn")
    ...
    redraw() {
      ...
      let labels = this.data.labels.map((v) => v);
      labels = this.slice ? labels.slice(this.slice) : labels;

      let valuesUsdPlus = [];

      if (this.usdPlusData?.length > 0) {
        labels.forEach((v) =>
          valuesUsdPlus.push(this.usdPlusData[v] ? this.usdPlusData[v] : null)
        );
      }
      ...
      let chartColor = this.$colorUtils.hexToRgb(
        this.getParams(this.etsData.chain).networkColor
      );
      ...
      let options = {
        ...
        colors: this.light
          ? [
              `rgba(${chartColor.r}, ${chartColor.g}, ${chartColor.b}, 0.3)`,
              "rgba(34, 171, 172, 0.3)"
            ]
          : [this.getParams(this.etsData.chain).networkColor, "#0d4444"],
      ...
      this.chart = new ApexCharts(
        document.querySelector("#line-chart-apy"),
        options
      );
      this.chart.render();
    }
